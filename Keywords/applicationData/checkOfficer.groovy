...
public class checkOfficer {
	...
	public countOfficerLookup(Sql instance, String officeLoc){
		Integer countData
		instance.eachRow(("select count(*) from ref_user_role rur WITH(NOLOCK) join ref_job_title rjt WITH(NOLOCK) on rur.REF_JOB_TITLE_ID = rjt.REF_JOB_TITLE_ID join ref_office ro WITH(NOLOCK) on ro.REF_OFFICE_ID = rur.REF_OFFICE_ID JOIN REF_USER RU ON RUR.REF_USER_ID = RU.REF_USER_ID join ref_emp re on re.ref_emp_id = ru.ref_emp_id where job_title_code IN ('SALES_PERSON','CMO') and office_name = '"+officeLoc+"' and rur.is_active=1"), { def row ->
			countData = (row[0])
		})
		return countData
	}
