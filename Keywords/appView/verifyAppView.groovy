...
public class verifyAppView {
	...
	public checkFinancialDetailPersonal (Sql instance, String appno, String name, String date){
		String appdata
		ArrayList <String> listappdata = new ArrayList<String>()
		instance.eachRow(("SELECT CASE WHEN CAST(MONTHLY_INCOME_AMT as VARCHAR(25)) = '0.00' OR CAST(MONTHLY_INCOME_AMT as VARCHAR(25)) IS NULL THEN '-' ELSE CAST(MONTHLY_INCOME_AMT as VARCHAR(25)) END AS MONINCOME, CASE WHEN CAST(MONTHLY_EXPENSE_AMT AS VARCHAR(25)) = '0.00' OR CAST(MONTHLY_EXPENSE_AMT AS VARCHAR(25)) IS NULL THEN '-' ELSE CAST(MONTHLY_EXPENSE_AMT AS VARCHAR(25)) END AS MONEXPN, ISNULL(rml.REF_MASTER_NAME,'') AS SOURCEINCOME, CASE WHEN CAST(MONTHLY_INSTALLMENT_AMT AS VARCHAR(25)) = '0.00' OR CAST(MONTHLY_INSTALLMENT_AMT AS VARCHAR(25)) IS NULL THEN '-' ELSE CAST(MONTHLY_INSTALLMENT_AMT AS VARCHAR(25)) END AS MONTHINST, CASE WHEN CAST(SPOUSE_MONTHLY_INCOME_AMT AS VARCHAR(25)) = '0.00' OR CAST(SPOUSE_MONTHLY_INCOME_AMT AS VARCHAR(25)) IS NULL THEN '-' ELSE CAST(SPOUSE_MONTHLY_INCOME_AMT AS VARCHAR(25)) END AS SPOUINC, CASE WHEN CAST(IS_JOIN_INCOME as VARCHAR(25)) = '1' THEN 'YES' WHEN CAST(IS_JOIN_INCOME as VARCHAR(25)) = '0' THEN 'NO' ELSE CAST(IS_JOIN_INCOME as VARCHAR(25)) END AS JOININC, CASE WHEN CAST(OTHER_INCOME_AMT as VARCHAR(25)) = '0.00' OR CAST(OTHER_INCOME_AMT as VARCHAR(25)) IS NULL THEN '-' ELSE CAST(OTHER_INCOME_AMT as VARCHAR(25)) END AS OTHINCOME, CASE WHEN CAST(OTHER_MONTHLY_INSTALLMENT_AMT AS VARCHAR(25)) = '0.00' OR CAST(OTHER_MONTHLY_INSTALLMENT_AMT AS VARCHAR(25)) IS NULL THEN '-' ELSE CAST(OTHER_MONTHLY_INSTALLMENT_AMT AS VARCHAR(25)) END AS OTHMONINS, CASE WHEN CAST(TOTAL_INCOME_AMT as VARCHAR(25)) = '0.00' OR CAST(TOTAL_INCOME_AMT as VARCHAR(25)) IS NULL THEN '-' ELSE CAST(TOTAL_INCOME_AMT as VARCHAR(25)) END AS TOTALMONTHINC, CASE WHEN CAST(MONTHLY_EXPENSE_AMT AS VARCHAR(25)) = '0.00' OR CAST(MONTHLY_EXPENSE_AMT AS VARCHAR(25)) IS NULL THEN '-' ELSE CAST(MONTHLY_EXPENSE_AMT AS VARCHAR(25)) END AS TOTALEXP, CASE WHEN CAST(NETT_INCOME_AMT AS VARCHAR(25)) = '0.00' OR CAST(NETT_INCOME_AMT AS VARCHAR(25)) IS NULL THEN '-' ELSE CAST(NETT_INCOME_AMT AS VARCHAR(25)) END AS NETTINC FROM APP_CUST_PERSONAL_FIN_DATA acpfd WITH(NOLOCK) JOIN APP_CUST_PERSONAL acp WITH(NOLOCK) ON acp.APP_CUST_PERSONAL_ID = acpfd.APP_CUST_PERSONAL_ID JOIN APP_CUST ac WITH(NOLOCK) ON ac.APP_CUST_ID = acp.APP_CUST_ID JOIN APP a WITH(NOLOCK) ON a.APP_ID = ac.APP_ID JOIN REF_MASTER_LOS rml WITH(NOLOCK) ON rml.REF_MASTER_CODE = acpfd.MR_SOURCE_OF_INCOME_TYPE_CODE WHERE a.APP_NO = '"+ appno +"' AND ac.CUST_NAME = '"+ name +"' AND FORMAT(acpfd.DATE_AS_OF , 'dd-MMM-yyyy') = '"+ date +"'"), {  row ->
