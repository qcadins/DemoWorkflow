...
public class checkReferantor {
	...
	public countReferantorLookup(Sql instance, String refCategory, String originOfficeName){
		...
		if(refCategory.equalsIgnoreCase("Customer")){
			instance.eachRow("select count(*) from cust a join cust_addr b on a.cust_id = b.cust_id where mr_cust_addr_type_code = 'legal'", { def row ->
				countData = row[0]
			})
		}
		else if(refCategory.equalsIgnoreCase("Agency")){
			instance.eachRow("select count(*) from vendor a join vendor_addr b on a.vendor_id = b.vendor_id join vendor_office_mbr c on a.vendor_id = c.vendor_id join ref_office d on c.ref_office_id = d.ref_office_id where mr_addr_type_code = 'legal'  and a.is_active=1 and (mr_vendor_category_code = 'AGENCY_PERSONAL' or mr_vendor_category_code = 'AGENCY_COMPANY') and office_name = '"+originOfficeName+"'", { def row ->
				countData = row[0]
			})
		}
		else{
			instance.eachRow("select count(*) from ref_emp where is_active=1", { def row ->
				countData = row[0]
			})
		}
		return countData
	...
	public checkBankAccountDDL(Sql instance, String refCategory, String officeName, String refCode){
		...
		if(refCategory.equalsIgnoreCase("Customer")){
			instance.eachRow("select [Bank Account] = bank_name+' - '+bank_acc_name+' - '+bank_acc_No from cust a join cust_addr b on a.cust_id = b.cust_id join cust_bank_acc c on a.cust_id = c.cust_id join ref_bank d on c.ref_bank_id = d.ref_bank_id where mr_cust_addr_type_code = 'legal' and c.is_active = 1 and cust_no = '"+refCode+"'", { def row ->
