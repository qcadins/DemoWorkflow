...
public class CustomerDataVerif {
	...
	public GuarantorDataStoreDBCompany (Sql instance, String appno, String name){
		String guarantordata
		instance.eachRow(("SELECT rml.REF_MASTER_NAME AS HEADER ,CUST_NAME AS HEADER ,TAX_ID_NO AS HEADER ,MR_COMPANY_TYPE_CODE AS HEADER ,MR_CUST_MODEL_CODE AS HEADER, ADDR AS HEADER, AREA_CODE_4 AS HEADER, AREA_CODE_3 AS HEADER, ZIPCODE AS HEADER, AREA_CODE_2 AS HEADER, AREA_CODE_1 AS HEADER, CITY AS HEADER, f.REF_MASTER_NAME AS HEADER FROM APP_CUST ac WITH(NOLOCK) JOIN app a WITH(NOLOCK) ON ac.APP_ID = a.APP_ID JOIN APP_CUST_COMPANY acp WITH(NOLOCK) ON ac.APP_CUST_ID = acp.APP_CUST_ID JOIN APP_CUST_ADDR aca WITH(NOLOCK) ON ac.APP_CUST_ID = aca.APP_CUST_ID JOIN REF_MASTER_LOS rml WITH(NOLOCK) ON rml.REF_MASTER_CODE = ac.MR_CUST_RELATIONSHIP_CODE JOIN REF_MASTER_LOS f WITH(NOLOCK) ON f.REF_MASTER_CODE = aca.MR_HOUSE_OWNERSHIP_CODE where APP_NO = '"+ appno +"' AND CUST_NAME = '"+ name +"'AND IS_GUARANTOR = 1 AND MR_CUST_ADDR_TYPE_CODE = 'LEGAL' AND MR_CUST_TYPE_CODE = 'COMPANY' AND rml.REF_MASTER_TYPE_CODE = 'GUAR_COMPANY_RELATIONSHIP' AND f.REF_MASTER_TYPE_CODE = 'BUILDING_OWNERSHIP'"), {  row ->
			guarantordata = (row)
		})
		return guarantordata
	}
	...
	public CustomerDataStoreDBCompany (Sql instance, String appno, String name){
		String customerdata
		instance.eachRow(("SELECT CUST_NAME AS HEADER ,TAX_ID_NO AS HEADER ,MR_COMPANY_TYPE_CODE AS HEADER ,MR_CUST_MODEL_CODE AS HEADER, ADDR AS HEADER, AREA_CODE_4 AS HEADER, AREA_CODE_3 AS HEADER, ZIPCODE AS HEADER, AREA_CODE_2 AS HEADER, AREA_CODE_1 AS HEADER, CITY AS HEADER, rml.REF_MASTER_NAME AS HEADER FROM APP_CUST ac WITH(NOLOCK) JOIN app a WITH(NOLOCK) ON ac.APP_ID = a.APP_ID JOIN APP_CUST_COMPANY acc WITH(NOLOCK) ON ac.APP_CUST_ID = acc.APP_CUST_ID JOIN APP_CUST_ADDR aca WITH(NOLOCK) ON ac.APP_CUST_ID = aca.APP_CUST_ID JOIN REF_MASTER_LOS rml WITH(NOLOCK) ON rml.REF_MASTER_CODE = aca.MR_HOUSE_OWNERSHIP_CODE where APP_NO = '"+ appno +"' AND CUST_NAME = '"+ name +"' AND IS_CUSTOMER = 1 AND MR_CUST_ADDR_TYPE_CODE = 'LEGAL' AND MR_CUST_TYPE_CODE = 'COMPANY' AND rml.REF_MASTER_TYPE_CODE = 'BUILDING_OWNERSHIP'"), {  row ->
	...
	public NAP2TermConditionStoreDB (Sql instance, String appno){
		...
		instance.eachRow(("SELECT IS_CHECKED, FORMAT(PROMISED_DT, 'MM/dd/yyy'), FORMAT(EXPIRED_DT, 'MM/dd/yyyy'), IS_WAIVED FROM APP_TC at WITH(NOLOCK)JOIN APP a WITH(NOLOCK) ON a.APP_ID = at.APP_ID WHERE APP_NO = '"+ appno +"'"), {  row ->
