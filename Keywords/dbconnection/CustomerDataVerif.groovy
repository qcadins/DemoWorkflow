...
public class CustomerDataVerif {
	...
	public CustomerDataStoreDBCompany (Sql instance, String appno, String name){
		String customerdata
		instance.eachRow(("SELECT CUST_NAME AS HEADER ,TAX_ID_NO AS HEADER ,MR_COMPANY_TYPE_CODE AS HEADER ,MR_CUST_MODEL_CODE AS HEADER, ADDR AS HEADER, AREA_CODE_4 AS HEADER, AREA_CODE_3 AS HEADER, ZIPCODE AS HEADER, AREA_CODE_2 AS HEADER, AREA_CODE_1 AS HEADER, CITY AS HEADER, rml.REF_MASTER_NAME AS HEADER FROM APP_CUST ac WITH(NOLOCK) JOIN app app ON ac.APP_ID = app.APP_ID JOIN APP_CUST_COMPANY acc ON ac.APP_CUST_ID = acc.APP_CUST_ID JOIN APP_CUST_ADDR addr ON ac.APP_CUST_ID = addr.APP_CUST_ID JOIN REF_MASTER_LOS rml ON rml.REF_MASTER_CODE = addr.MR_HOUSE_OWNERSHIP_CODE where APP_NO = '"+ appno +"' AND CUST_NAME = '"+ name +"' AND IS_CUSTOMER = 1 AND MR_CUST_ADDR_TYPE_CODE = 'LEGAL' AND MR_CUST_TYPE_CODE = 'COMPANY' AND rml.REF_MASTER_TYPE_CODE = 'BUILDING_OWNERSHIP'"), {  row ->
	...
	public NAP2TabApplicationStoreDB (Sql instance, String appno, String name){
		...
		instance.eachRow(("SELECT [APPSRCNAME] AS HEADER, [FIRST_INST_TYPE] AS HEADER, [PAYMENTFREQ] AS HEADER, [TENOR] AS HEADER, [DPSRCPAYMENT] AS HEADER, [INTRSTTYPE] AS HEADER, [FLOATINGPERIOD] AS HEADER, [INST_SCHM] AS HEADER, [WOP] AS HEADER, [CUST_NOTIF_OPT] AS HEADER, [CSP_USL_AML] AS HEADER, [ADDR] AS HEADER, [RT] AS HEADER, [RW] AS HEADER, [ZIPCODE] AS HEADER, [KECAMATAN] AS HEADER, [KELURAHAN] AS HEADER, [KOTA] AS HEADER, [PHN1AREA] AS HEADER, [PHN1] AS HEADER, [PHN1EXT] AS HEADER, [PHN2AREA] AS HEADER, [PHN2] AS HEADER, [PHN2EXT] AS HEADER, [PHN3AREA] AS HEADER, [PHN3] AS HEADER, [PHN3EXT] AS HEADER, [FAXAREA] AS HEADER, [FAX] AS HEADER, [CHARACTERISTIC_OF_CREDIT] AS HEADER, [PREVAGGRMT] AS HEADER, [WAY_OF_RESTRUCTURE] AS HEADER, [SLIKECO] AS HEADER FROM (SELECT [APPSRCNAME], [PAYMENTFREQ], [TENOR], [DPSRCPAYMENT], [FLOATINGPERIOD], [ADDR], [RT], [RW], [ZIPCODE], [KECAMATAN], [KELURAHAN], [KOTA], [PHN1AREA], [PHN1], [PHN1EXT], [PHN2AREA], [PHN2], [PHN2EXT], [PHN3AREA], [PHN3], [PHN3EXT], [FAXAREA], [FAX], [PREVAGGRMT], [SLIKECO],  mastername.Code, REF_MASTER_NAME FROM ( select [APPSRCNAME], [PAYMENTFREQ], [TENOR], [DPSRCPAYMENT], [FLOATINGPERIOD], [ADDR], [RT], [RW], [ZIPCODE], [KECAMATAN], [KELURAHAN], [KOTA], [PHN1AREA], [PHN1], [PHN1EXT], [PHN2AREA], [PHN2], [PHN2EXT], [PHN3AREA], [PHN3], [PHN3EXT], [FAXAREA], [FAX], [PREVAGGRMT], [SLIKECO], [Code], value FROM ( SELECT appsrc.APP_SRC_NAME AS [APPSRCNAME], MR_FIRST_INST_TYPE_CODE AS 'FIRST_INST_TYPE', PAY_FREQ_CODE AS [PAYMENTFREQ], TENOR AS [TENOR], rml.REF_MASTER_NAME AS [DPSRCPAYMENT], CAST(fin.INTEREST_TYPE as nvarchar(50)) AS 'INTRSTTYPE', FLOATING_PERIOD_CODE AS [FLOATINGPERIOD], MR_INST_SCHEME_CODE AS 'INST_SCHM', MR_WOP_CODE AS 'WOP', MR_CUST_NOTIFY_OPT_CODE AS 'CUST_NOTIF_OPT', MR_INST_SRC_PAYMENT_CODE AS 'CSP_USL_AML', addr.ADDR AS [ADDR], addr.AREA_CODE_3 AS [RT], addr.AREA_CODE_4 AS [RW], addr.ZIPCODE AS [ZIPCODE], addr.AREA_CODE_2 AS [KECAMATAN], addr.AREA_CODE_1 AS [KELURAHAN], addr.CITY AS [KOTA], addr.PHN_AREA_1 AS [PHN1AREA], addr.PHN_1 AS [PHN1], addr.PHN_EXT_1 AS [PHN1EXT], addr.PHN_AREA_2 AS [PHN2AREA], addr.PHN_2 AS [PHN2], addr.PHN_EXT_2 AS [PHN2EXT] , addr.PHN_AREA_3 AS [PHN3AREA], addr.PHN_3 AS [PHN3], addr.PHN_EXT_3 AS [PHN3EXT], addr.FAX_AREA AS [FAXAREA], addr.FAX AS [FAX], MR_CHARACTERISTIC_OF_CREDIT_CODE AS 'CHARACTERISTIC_OF_CREDIT', agr.PREV_AGRMNT_NO AS [PREVAGGRMT], MR_WAY_OF_RESTRUCTURE_CODE AS 'WAY_OF_RESTRUCTURE', agr.MR_SLIK_SEC_ECO_CODE AS [SLIKECO] FROM APP app WITH(NOLOCK) JOIN APP_CUST ac WITH(NOLOCK) ON app.APP_ID = ac.APP_ID JOIN APP_CUST_ADDR addr WITH(NOLOCK) ON ac.APP_CUST_ID = addr.APP_CUST_ID JOIN APP_GOVERMENT_REGULATION agr WITH(NOLOCK) ON agr.APP_ID = app.APP_ID JOIN APP_FIN_DATA fin WITH(NOLOCK) ON fin.APP_ID = ac.APP_ID JOIN REF_APP_SRC appsrc WITH(NOLOCK) ON appsrc.APP_SRC_CODE = app.MR_APP_SOURCE_CODE JOIN REF_MASTER_LOS rml WITH(NOLOCK) ON rml.REF_MASTER_CODE = agr.MR_DP_SRC_PAYMENT_CODE WHERE APP_NO = '"+ appno +"' AND addr.MR_CUST_ADDR_TYPE_CODE = 'MAILING' AND CUST_NAME = '"+ name +"') as Orig unpivot (value for [Code] in ([FIRST_INST_TYPE],[WOP],[INTRSTTYPE],[INST_SCHM],[CUST_NOTIF_OPT],[CSP_USL_AML],[CHARACTERISTIC_OF_CREDIT],[WAY_OF_RESTRUCTURE]) )as unpiv) as mastername JOIN REF_MASTER_LOS rf WITH(NOLOCK) ON rf.REF_MASTER_Code = mastername.value ) AS ref  PIVOT (MAX(ref.REF_MASTER_NAME) for [Code] in ([FIRST_INST_TYPE],[WOP],[INTRSTTYPE],[INST_SCHM],[CUST_NOTIF_OPT],[CSP_USL_AML],[CHARACTERISTIC_OF_CREDIT],[WAY_OF_RESTRUCTURE])) as piv"), {  row ->
	...
	public NAP2TabApplicationAttrStoreDB (Sql instance, String appno){
		...
		instance.eachRow(("SELECT ATTR_VALUE FROM APP_ATTR_CONTENT aac JOIN APP app ON app.APP_ID = aac.APP_ID WHERE APP_NO = '"+ appno +"'"), {  row ->
	...
	public NAP2TabAssetSupplierInfoStoreDB (Sql instance, String appno){
		String applicationdata
		instance.eachRow(("SELECT SUPPL_CODE AS HEADER, SUPPL_NAME AS HEADER, se.SUPPL_EMP_NAME AS HEADER, SUBQ.SUPPL_EMP_NAME AS HEADER FROM APP_ASSET_SUPPL_EMP se JOIN APP_ASSET aaw WITH(NOLOCK) ON aaw.APP_ASSET_ID = se.APP_ASSET_ID JOIN APP app WITH(NOLOCK) ON app.APP_ID = aaw.APP_ID, (SELECT SUPPL_EMP_NAME FROM APP_ASSET_SUPPL_EMP se JOIN APP_ASSET aaw WITH(NOLOCK) ON aaw.APP_ASSET_ID = se.APP_ASSET_ID JOIN APP app WITH(NOLOCK) ON app.APP_ID = aaw.APP_ID WHERE APP_NO = '"+ appno +"' AND MR_SUPPL_EMP_POSITION_CODE = 'SALES_PERSON') as SUBQ WHERE APP_NO = '"+ appno +"' AND se.MR_SUPPL_EMP_POSITION_CODE = 'ADMIN_HEAD'"), {  row ->
