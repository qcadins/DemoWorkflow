...
public class CustomerDataVerif {
	...
	public CustomerDataStoreDBPersonal (Sql instance, String appno){
		String customerdata
		ArrayList<String> listcustomerdata = new ArrayList<>()
		//instance.eachRow(("SELECT b.PROD_OFFERING_CODE , Cust_name , BIRTH_PLACE , h.DESCR , FORMAT(ID_EXPIRED_DT, 'MM/dd/yyyy') , MR_MARITAL_STAT_CODE , MOBILE_PHN_NO_1 , f.DESCR , MR_GENDER_CODE , FORMAT(BIRTH_DT, 'MM/dd/yyyy') , ID_NO , TAX_ID_NO , MOTHER_MAIDEN_NAME , EMAIL_1 , e.ATTR_VALUE , SUBQ.ATTR_VALUE , ADDR , AREA_CODE_4 , AREA_CODE_3 , ZIPCODE , AREA_CODE_2 , AREA_CODE_1 , CITY , g.DESCR  FROM APP_CUST a JOIN app b ON a.APP_ID = b.APP_ID JOIN APP_CUST_PERSONAL c ON a.APP_CUST_ID = c.APP_CUST_ID JOIN APP_CUST_ADDR d ON a.APP_CUST_ID = d.APP_CUST_ID JOIN APP_CUST_ATTR_CONTENT e ON e.APP_CUST_ID = a.APP_CUST_ID JOIN FOUNDATION.dbo.REF_MASTER f ON a.MR_CUST_MODEL_CODE = f.MASTER_CODE JOIN FOUNDATION.dbo.REF_MASTER g ON d.MR_HOUSE_OWNERSHIP_CODE = g.MASTER_CODE JOIN FOUNDATION.dbo.REF_MASTER h ON a.MR_ID_TYPE_CODE = h.MASTER_CODE, (SELECT ATTR_VALUE, a.APP_ID FROM APP_CUST_ATTR_CONTENT g JOIN APP_CUST a ON g.APP_CUST_ID = a.APP_CUST_ID JOIN app b ON a.APP_ID = b.APP_ID WHERE APP_NO = '"+ appno +"'AND IS_CUSTOMER = 1 AND g.REF_ATTR_CODE = 'AUTH_AML') as SUBQ WHERE a.APP_ID = SUBQ.APP_ID AND IS_CUSTOMER = 1 AND MR_CUST_ADDR_TYPE_CODE = 'LEGAL' AND MR_CUST_TYPE_CODE = 'PERSONAL' AND e.REF_ATTR_CODE = 'AML_CUST_DEPARTMENT' AND f.REF_MASTER_TYPE_CODE = 'CUST_MODEL' AND g.REF_MASTER_TYPE_CODE = 'BUILDING_OWNERSHIP' AND h.REF_MASTER_TYPE_CODE = 'ID_TYPE'"), {  row ->
		instance.eachRow(("SELECT ISNULL([ProductOffering],'') AS PROD_OFFERING, ISNULL([CustName],'') AS CUST_NAME, ISNULL([BirthPlace],'') AS BIRTH_PLACE, ISNULL([ID_TYPE],'') AS ID_TYPE,ISNULL([IDExpiredDate],'') AS ID_EXPIRED_DATE, ISNULL([MaritalStatus],'') AS MARITALSTAT, ISNULL([MobilePHN1],'') AS MOBILE1, ISNULL([CUST_MODEL],'') AS CUST_MODEL, ISNULL([Gender],'') AS GENDER, ISNULL([BirthDate],'') AS BIRTH_DATE, ISNULL([IdNo],'') AS ID_NO, ISNULL([TaxID],'') AS TAX_ID, ISNULL([MotherMaidenName],'') AS MOTHER_MAIDEN_NAME,ISNULL([Email1],'') AS EMAIL1, ISNULL([Addr],'') AS ADDR, ISNULL([RT],'') AS RT, ISNULL([RW],'') AS RW, ISNULL([Zipcode],'') AS ZIPCODE, ISNULL([Kecamatan],'') AS KECAMATAN, ISNULL([Kelurahan],'') AS KELURAHAN, ISNULL([Kota],'') AS KOTA, ISNULL([BUILDING_OWNERSHIP],'') AS OWNERSHIP FROM (SELECT [ProductOffering], [CustName], [BirthPlace], [IDExpiredDate], [MaritalStatus], [MobilePHN1], [Gender], [BirthDate], [IdNo], [TaxID], [MotherMaidenName], [Email1], [Addr], [RT], [RW], [Zipcode], [Kecamatan], [Kelurahan], [Kota], mastername.Code, DESCR FROM (select [ProductOffering], [CustName], [BirthPlace], [IDExpiredDate], [MaritalStatus], [MobilePHN1], [Gender], [BirthDate], [IdNo], [TaxID], [MotherMaidenName], [Email1], [Addr], [RT], [RW], [Zipcode], [Kecamatan], [Kelurahan], [Kota], [Code], value FROM (SELECT a.PROD_OFFERING_CODE AS [ProductOffering], Cust_name AS [CustName], BIRTH_PLACE AS [BirthPlace], ac.MR_ID_TYPE_CODE AS 'ID_TYPE', FORMAT(ID_EXPIRED_DT, 'MM/dd/yyyy') AS [IDExpiredDate], MR_MARITAL_STAT_CODE AS [MaritalStatus], MOBILE_PHN_NO_1 AS [MobilePHN1], ac.MR_CUST_MODEL_CODE AS 'CUST_MODEL', MR_GENDER_CODE AS [Gender], FORMAT(BIRTH_DT, 'MM/dd/yyyy') AS [BirthDate], ID_NO AS [IdNo], TAX_ID_NO AS [TaxID], MOTHER_MAIDEN_NAME AS [MotherMaidenName], EMAIL_1 AS [Email1],  ADDR AS [Addr], AREA_CODE_4 AS [RT], AREA_CODE_3 AS [RW], ZIPCODE AS [Zipcode], AREA_CODE_2 AS [Kecamatan], AREA_CODE_1 AS [Kelurahan], CITY AS [Kota], addr.MR_HOUSE_OWNERSHIP_CODE AS 'BUILDING_OWNERSHIP' FROM APP_CUST ac JOIN APP a WITH(NOLOCK) ON ac.APP_ID = a.APP_ID JOIN APP_CUST_PERSONAL acp WITH(NOLOCK) ON ac.APP_CUST_ID = acp.APP_CUST_ID JOIN APP_CUST_ADDR addr WITH(NOLOCK) ON ac.APP_CUST_ID = addr.APP_CUST_ID WHERE ac.IS_CUSTOMER = 1 AND APP_NO = '"+ appno +"' AND MR_CUST_ADDR_TYPE_CODE = 'LEGAL' AND MR_CUST_TYPE_CODE = 'PERSONAL') as Orig unpivot (value for [Code] in ([ID_TYPE],[CUST_MODEL],[BUILDING_OWNERSHIP]))as unpiv) as mastername JOIN FOUNDATION.dbo.REF_MASTER rf WITH(NOLOCK) ON rf.MASTER_CODE = mastername.value WHERE rf.IS_ACTIVE = '1') AS ref PIVOT (MAX(ref.DESCR) for [Code] in ([ID_TYPE],[CUST_MODEL],[BUILDING_OWNERSHIP])) as piv"), {  row ->
	...
	public GuarantorDataStoreDBPersonal (Sql instance, String appno, String name){
		...
		ArrayList<String> listcustomerdata = new ArrayList<>()
		//		instance.eachRow(("SELECT MR_CUST_RELATIONSHIP_CODE  ,Cust_name , BIRTH_PLACE , f.DESCR , FORMAT(ID_EXPIRED_DT, 'MM/dd/yyyy') , MR_MARITAL_STAT_CODE , MOBILE_PHN_NO_1 , g.DESCR , MR_GENDER_CODE , FORMAT(BIRTH_DT, 'MM/dd/yyyy') , ID_NO , TAX_ID_NO , MOTHER_MAIDEN_NAME , EMAIL_1 , e.ATTR_VALUE , SUBQ.ATTR_VALUE , ADDR , AREA_CODE_4 , AREA_CODE_3 , ZIPCODE , AREA_CODE_2 , AREA_CODE_1 , CITY , h.DESCR  FROM APP_CUST a JOIN app b ON a.APP_ID = b.APP_ID JOIN APP_CUST_PERSONAL c ON a.APP_CUST_ID = c.APP_CUST_ID JOIN APP_CUST_ADDR d ON a.APP_CUST_ID = d.APP_CUST_ID JOIN APP_CUST_ATTR_CONTENT e ON e.APP_CUST_ID = a.APP_CUST_ID JOIN FOUNDATION.dbo.REF_MASTER f ON a.MR_ID_TYPE_CODE = f.MASTER_CODE JOIN FOUNDATION.dbo.REF_MASTER g ON a.MR_CUST_MODEL_CODE = g.MASTER_CODE JOIN FOUNDATION.dbo.REF_MASTER h ON d.MR_HOUSE_OWNERSHIP_CODE = h.MASTER_CODE, (SELECT ATTR_VALUE, a.APP_ID FROM APP_CUST_ATTR_CONTENT g JOIN APP_CUST a ON g.APP_CUST_ID = a.APP_CUST_ID JOIN app b ON a.APP_ID = b.APP_ID WHERE APP_NO = '"+ appno +"' AND IS_GUARANTOR = 1 AND g.REF_ATTR_CODE = 'AUTH_AML') as SUBQ WHERE APP_NO = '" + appno +"' AND IS_GUARANTOR = 1 AND MR_CUST_ADDR_TYPE_CODE = 'LEGAL' AND MR_CUST_TYPE_CODE = 'PERSONAL' AND e.REF_ATTR_CODE = 'AML_CUST_DEPARTMENT' AND f.REF_MASTER_TYPE_CODE = 'ID_TYPE' AND g.REF_MASTER_TYPE_CODE = 'CUST_MODEL' AND h.REF_MASTER_TYPE_CODE = 'BUILDING_OWNERSHIP'"), {  row ->
		instance.eachRow(("SELECT ISNULL([Relationship],'') AS RELATIONSHIP , ISNULL([CustName],'') AS CUST_NAME, ISNULL([BirthPlace],'') AS BIRTH_PLACE, ISNULL([ID_TYPE],'') AS ID_TYPE ,ISNULL([IDExpiredDate],'') AS ID_EXP_DATE, ISNULL([MaritalStatus],'') AS MARTIAL_STAT, ISNULL([MobilePHN1],'') AS MOBILE1, ISNULL([CUST_MODEL],'') AS CUST_MODEL, ISNULL([Gender],'') AS GENDER, ISNULL([BirthDate],'') AS BIRTH_DATE , ISNULL([Idno],'') AS ID_NO , ISNULL([TaxID],'') AS TAX_ID, ISNULL([MotherMaidenName],'') AS MOTHER_MAIDEN_NAME , ISNULL([Email1],'') AS EMAIL1, ISNULL([Addr],'') AS ADDR, ISNULL([RT],'') AS RT, ISNULL([RW],'') AS RW, ISNULL([Zipcode],'') AS ZIPCODE, ISNULL([Kecamatan],'') AS KECAMATAN , ISNULL([Kelurahan],'') AS KELURAHAN , ISNULL([Kota],'') AS KOTA, ISNULL([BUILDING_OWNERSHIP],'') AS OWNERSHIP FROM (SELECT [Relationship], [CustName], [BirthPlace], [IDExpiredDate], [MaritalStatus], [MobilePHN1], [Gender], [BirthDate], [Idno], [TaxID], [MotherMaidenName], [Email1], [Addr], [RT], [RW], [Zipcode], [Kecamatan], [Kelurahan], [Kota], mastername.Code, DESCR FROM (select [Relationship], [CustName], [BirthPlace], [IDExpiredDate], [MaritalStatus], [MobilePHN1], [Gender], [BirthDate], [Idno], [TaxID], [MotherMaidenName], [Email1], [Addr], [RT], [RW], [Zipcode], [Kecamatan], [Kelurahan], [Kota], [Code], value FROM (SELECT MR_CUST_RELATIONSHIP_CODE AS [Relationship] ,Cust_name AS [CustName], BIRTH_PLACE AS [BirthPlace], ac.MR_ID_TYPE_CODE AS 'ID_TYPE', FORMAT(ID_EXPIRED_DT, 'MM/dd/yyyy') AS [IDExpiredDate], MR_MARITAL_STAT_CODE AS [MaritalStatus], MOBILE_PHN_NO_1 AS [MobilePHN1], MR_CUST_MODEL_CODE AS 'CUST_MODEL', MR_GENDER_CODE AS [Gender], FORMAT(BIRTH_DT, 'MM/dd/yyyy') AS [BirthDate], ID_NO AS [Idno], TAX_ID_NO AS [TaxID], MOTHER_MAIDEN_NAME AS [MotherMaidenName], EMAIL_1 AS [Email1], ADDR AS [Addr], AREA_CODE_4 AS [RT], AREA_CODE_3 AS [RW], ZIPCODE AS [Zipcode], AREA_CODE_2 AS [Kecamatan], AREA_CODE_1 AS [Kelurahan], CITY AS [Kota], aca.MR_HOUSE_OWNERSHIP_CODE AS 'BUILDING_OWNERSHIP' FROM APP_CUST ac WITH(NOLOCK) JOIN app a WITH(NOLOCK) ON ac.APP_ID = a.APP_ID JOIN APP_CUST_PERSONAL acp WITH(NOLOCK) ON ac.APP_CUST_ID = acp.APP_CUST_ID JOIN APP_CUST_ADDR aca WITH(NOLOCK) ON ac.APP_CUST_ID = aca.APP_CUST_ID where APP_NO = '"+ appno +"' AND IS_GUARANTOR = 1 AND Cust_name = '"+ name +"' AND aca.MR_CUST_ADDR_TYPE_CODE = 'LEGAL') as Orig unpivot (value for [Code] in ([ID_TYPE],[CUST_MODEL],[BUILDING_OWNERSHIP]))as unpiv) as mastername JOIN FOUNDATION.dbo.REF_MASTER rf WITH(NOLOCK) ON rf.MASTER_CODE = mastername.value WHERE rf.IS_ACTIVE = '1') AS ref PIVOT (MAX(ref.DESCR) for [Code] in ([ID_TYPE],[CUST_MODEL],[BUILDING_OWNERSHIP])) as piv"), {  row ->
			ResultSetMetaData rsmd = row.getMetaData()
			colmcount = rsmd.getColumnCount()
	...
	public CustomerDataStoreDBCompany (Sql instance, String appno, String name){
		String customerdata
		ArrayList<String> listcustomerdata = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL(ac.CUST_NAME,'') AS CUST_NAME,ISNULL(TAX_ID_NO,'') AS TAX_ID_NO ,ISNULL(MR_COMPANY_TYPE_CODE,'') AS COMPANY_TYPE_CODE ,ISNULL(SUBQ.DESCR,'') AS CUST_MODEL_CODE , ISNULL(ADDR,'') AS ADDR, ISNULL(AREA_CODE_4,'') AS AREA4, ISNULL(AREA_CODE_3,'') AS AREA3, ISNULL(ZIPCODE,'') AS ZIPCODE, ISNULL(AREA_CODE_2,'') AS AREA2, ISNULL(AREA_CODE_1,'') AS AREA1, ISNULL(CITY,'') AS CITY, ISNULL(rml.DESCR,'') AS OWNERSHIP FROM APP_CUST ac WITH(NOLOCK) JOIN app a WITH(NOLOCK) ON ac.APP_ID = a.APP_ID JOIN APP_CUST_COMPANY acc WITH(NOLOCK) ON ac.APP_CUST_ID = acc.APP_CUST_ID JOIN APP_CUST_ADDR aca WITH(NOLOCK) ON ac.APP_CUST_ID = aca.APP_CUST_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = aca.MR_HOUSE_OWNERSHIP_CODE,(SELECT rml.DESCR, a.APP_NO, ac.CUST_NAME FROM APP_CUST_COMPANY acc WITH(NOLOCK) JOIN APP_CUST ac WITH(NOLOCK) ON acc.APP_CUST_ID = ac.APP_CUST_ID JOIN APP a WITH(NOLOCK) ON a.APP_ID = ac.APP_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = ac.MR_CUST_MODEL_CODE WHERE REF_MASTER_TYPE_CODE = 'CUST_MODEL') as SUBQ where a.APP_NO = '"+ appno +"' AND ac.CUST_NAME = '"+ name +"' AND ac.IS_CUSTOMER = 1 AND aca.MR_CUST_ADDR_TYPE_CODE = 'LEGAL' AND MR_CUST_TYPE_CODE = 'COMPANY' AND rml.REF_MASTER_TYPE_CODE = 'BUILDING_OWNERSHIP' AND SUBQ.APP_NO = a.APP_NO AND SUBQ.CUST_NAME = ac.CUST_NAME AND rml.IS_ACTIVE = '1'"), {  row ->
	...
	//get tab application data store db
	@Keyword
	public NAP2TabApplicationStoreDBFL4W (Sql instance, String appno, String name){
		String applicationdata
		ArrayList<String> listapplicationdata = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL([APPSRCNAME],'') AS APPSOURCE, ISNULL([FIRST_INST_TYPE],'') AS FIRST_INST_TYPE, ISNULL([PAYMENTFREQ],'') AS PAYMENT_FREQ, ISNULL([TENOR],'') AS TENOR, ISNULL([MOU_CUST], '') AS MOU_CUST, ISNULL([DPSRCPAYMENT],'') AS DP_SRC_PAYMENT, ISNULL([INTRSTTYPE],'') AS INTEREST_TYPE, ISNULL([FLOATINGPERIOD],'') AS FLOATING_PERIOD, ISNULL([INST_SCHM],'') AS INST_SCHM, ISNULL([WOP],'') AS WAY_OF_PAYMENT, ISNULL([CUST_NOTIF_OPT],'') AS CUST_NOTIF, ISNULL([CSP_USL_AML],'') AS CSP_USL_AML, ISNULL([ADDR],'') AS ADDR, ISNULL([RW],'') AS RW, ISNULL([RT],'') AS RT, ISNULL([ZIPCODE],'') AS ZIPCODE, ISNULL([KECAMATAN],'') AS KECAMATAN, ISNULL([KELURAHAN],'') AS KELURAHAN, ISNULL([KOTA],'') AS KOTA, ISNULL([PHN1AREA],'') AS PHN1AREA, ISNULL([PHN1],'') AS PHN1, ISNULL([PHN1EXT],'') AS PHN1EXT, ISNULL([PHN2AREA],'') AS PHN2AREA, ISNULL([PHN2],'') AS PHN2, ISNULL([PHN2EXT],'') AS PHN2EXT, ISNULL([PHN3AREA],'') AS PHN3AREA, ISNULL([PHN3],'') AS PHN3, ISNULL([PHN3EXT],'') AS PHN3EXT, ISNULL([FAXAREA],'') AS FAXAREA, ISNULL([FAX],'') AS FAX, ISNULL([CHARACTERISTIC_OF_CREDIT],'') AS COC, ISNULL([PREVAGGRMT],'') AS PREV_AGRMNT, ISNULL([WAY_OF_RESTRUCTURE],'') AS WOR, ISNULL([SLIKECO],'') AS SLIK_ECO FROM (SELECT [APPSRCNAME], [PAYMENTFREQ], [TENOR], [FLOATINGPERIOD], [ADDR], [RT], [RW], [ZIPCODE], [KECAMATAN], [KELURAHAN], [KOTA], [PHN1AREA], [PHN1], [PHN1EXT], [PHN2AREA], [PHN2], [PHN2EXT], [PHN3AREA], [PHN3], [PHN3EXT], [FAXAREA], [FAX], [PREVAGGRMT], [SLIKECO], [MOU_CUST],  mastername.Code, DESCR FROM ( select [APPSRCNAME], [PAYMENTFREQ], [TENOR], [FLOATINGPERIOD], [ADDR], [RT], [RW], [ZIPCODE], [KECAMATAN], [KELURAHAN], [KOTA], [PHN1AREA], [PHN1], [PHN1EXT], [PHN2AREA], [PHN2], [PHN2EXT], [PHN3AREA], [PHN3], [PHN3EXT], [FAXAREA], [FAX], [PREVAGGRMT], [SLIKECO], [MOU_CUST], [Code], value FROM ( SELECT CASE WHEN a.MOU_CUST_ID IS NULL THEN '' ELSE mc.CUST_NAME END AS [MOU_CUST], ras.APP_SRC_NAME AS [APPSRCNAME], MR_FIRST_INST_TYPE_CODE AS 'FIRST_INST_TYPE', PAY_FREQ_CODE AS [PAYMENTFREQ], TENOR AS [TENOR], MR_DP_SRC_PAYMENT_CODE AS 'DPSRCPAYMENT', CAST(acpfd.INTEREST_TYPE as nvarchar(50)) AS 'INTRSTTYPE', FLOATING_PERIOD_CODE AS [FLOATINGPERIOD], MR_INST_SCHEME_CODE AS 'INST_SCHM', MR_WOP_CODE AS 'WOP', MR_CUST_NOTIFY_OPT_CODE AS 'CUST_NOTIF_OPT', MR_INST_SRC_PAYMENT_CODE AS 'CSP_USL_AML', aca.ADDR AS [ADDR], aca.AREA_CODE_3 AS [RT], aca.AREA_CODE_4 AS [RW], aca.ZIPCODE AS [ZIPCODE], aca.AREA_CODE_2 AS [KECAMATAN], aca.AREA_CODE_1 AS [KELURAHAN], aca.CITY AS [KOTA], aca.PHN_AREA_1 AS [PHN1AREA], aca.PHN_1 AS [PHN1], aca.PHN_EXT_1 AS [PHN1EXT], aca.PHN_AREA_2 AS [PHN2AREA], aca.PHN_2 AS [PHN2], aca.PHN_EXT_2 AS [PHN2EXT] , aca.PHN_AREA_3 AS [PHN3AREA], aca.PHN_3 AS [PHN3], aca.PHN_EXT_3 AS [PHN3EXT], aca.FAX_AREA AS [FAXAREA], aca.FAX AS [FAX], MR_CHARACTERISTIC_OF_CREDIT_CODE AS 'CHARACTERISTIC_OF_CREDIT', agr.PREV_AGRMNT_NO AS [PREVAGGRMT], MR_WAY_OF_RESTRUCTURE_CODE AS 'WAY_OF_RESTRUCTURE', agr.MR_SLIK_SEC_ECO_CODE AS [SLIKECO] FROM APP a WITH(NOLOCK) JOIN APP_CUST ac WITH(NOLOCK) ON a.APP_ID = ac.APP_ID JOIN APP_CUST_ADDR aca WITH(NOLOCK) ON ac.APP_CUST_ID = aca.APP_CUST_ID JOIN APP_GOVERMENT_REGULATION agr WITH(NOLOCK) ON agr.APP_ID = a.APP_ID JOIN APP_FIN_DATA acpfd WITH(NOLOCK) ON acpfd.APP_ID = ac.APP_ID JOIN REF_APP_SRC ras WITH(NOLOCK) ON ras.APP_SRC_CODE = a.MR_APP_SOURCE_CODE LEFT JOIN MOU_CUST mc WITH(NOLOCK) ON mc.MOU_CUST_ID = a.MOU_CUST_ID WHERE APP_NO = '"+ appno +"' AND aca.MR_CUST_ADDR_TYPE_CODE = 'MAILING' AND ac.CUST_NAME = '"+ name +"') as Orig unpivot (value for [Code] in ([FIRST_INST_TYPE],[WOP],[DPSRCPAYMENT],[INTRSTTYPE],[INST_SCHM],[CUST_NOTIF_OPT],[CSP_USL_AML],[CHARACTERISTIC_OF_CREDIT],[WAY_OF_RESTRUCTURE]) )as unpiv) as mastername JOIN FOUNDATION.dbo.REF_MASTER rf WITH(NOLOCK) ON rf.MASTER_CODE = mastername.value WHERE rf.IS_ACTIVE = '1' AND rf.REF_MASTER_TYPE_CODE IN ('FIRST_INST_TYPE','WOP','INTRSTTYPE','INST_SCHM','CUST_NOTIF_OPT','CSP_USL_AML','CHARACTERISTIC_OF_CREDIT','WAY_OF_RESTRUCTURE')) AS ref PIVOT(MAX(ref.DESCR) for [Code] in ([FIRST_INST_TYPE],[WOP],[DPSRCPAYMENT],[INTRSTTYPE],[INST_SCHM],[CUST_NOTIF_OPT],[CSP_USL_AML],[CHARACTERISTIC_OF_CREDIT],[WAY_OF_RESTRUCTURE])) as piv"), {  row ->
	...
	public BankAccountTabApplicationDataStoreDB (Sql instance, String appno){
		...
		instance.eachRow(("SELECT rbl.BANK_NAME , aoi.BANK_ACC_NO , aoi.BANK_ACC_NAME  FROM app a WITH(NOLOCK) JOIN APP_CUST ac WITH(NOLOCK) ON a.APP_ID = ac.APP_ID JOiN APP_CUST_BANK_ACC aoi WITH(NOLOCK) ON ac.APP_CUST_ID = aoi.APP_CUST_ID JOIN REF_BANK_LOS rbl WITH(NOLOCK) ON aoi.BANK_CODE = rbl.BANK_CODE WHERE APP_NO = '"+ appno +"' AND IS_CUSTOMER = 1"), {  row ->
	...
	//get tab collateral data store db
	@Keyword
	public NAP2TabCollateralStoreDB (Sql instance, String appno){
		String assetinfo
		ArrayList<String> listassetinfo = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL([COLLATERAL_ID], ''), ISNULL([COLLATERAL_STAT], ''), ISNULL([ASSET_TYPE_CODE], ''), ISNULL([FULL_ASSET_CODE], ''), ISNULL([FULL_ASSET_NAME], ''), ISNULL([MANUFACTURING_YEAR], ''), ISNULL([MR_COLLATERAL_CONDITION_CODE], ''), ISNULL([ASSET_USAGE], ''), ISNULL([ASSET_TAX_DT], ''), [COLLATERAL_VALUE_AMT], [COLLATERAL_PORTION_PRCNT], [COLLATERAL_PORTION_AMT], [REMAINING_PORTION_PRCNT], [COLLATERAL_NOTES], ISNULL([SERIAL_NO_1], ''), ISNULL([SERIAL_NO_2], ''), ISNULL([SERIAL_NO_3], ''), ISNULL([SERIAL_NO_4], ''), ISNULL([SERIAL_NO_5], ''), ISNULL([OWNERTYPE], '') AS OWNERTYPE, ISNULL([OWNERNAME], '') AS OWNERNAME, ISNULL([OWNERRELATIONSHIP], '') AS OWNER_RELATION, CASE WHEN OWNERTYPE = 'PERSONAL' THEN PROFNAME WHEN OWNERTYPE = 'COMPANY' THEN OWNERPROF END AS OWNER_PROF, ISNULL([ID_TYPE], '') AS IDTYPE, ISNULL([OWNERID], '') AS IDONWER, ISNULL([OWNERMOBILE], '') AS OWNER_MOBILE, ISNULL([OWNERADDR], '') AS OWNERADDR, ISNULL([OWNERRT], '') AS OWNERRT, ISNULL([OWNERRW], '') AS OWNERRW, ISNULL([OWNERZIPCODE], '') AS OWNERZIPCODE, ISNULL([OWNERKECAMATAN], '') AS OWNERKECAMATAN, ISNULL([OWNERKELURAHAN], '') AS OWNERKELURAHAN, ISNULL([OWNERKOTA], '') AS OWNERKOTA, ISNULL([LOCATIONADDR], '') AS LOCADDR, ISNULL([LOCATIONRT], '') AS LOCRT, ISNULL([LOCATIONRW], '') AS LOCRW, ISNULL([LOCATIONZIPCODE], '') AS LOCZIPCODE, ISNULL([LOCATIONKECAMATAN], '') AS LOCKECAMATAN, ISNULL([LOCATIONKELURAHAN], '') AS LOCKELURAHAN, ISNULL([LOCATIONKOTA], '') AS LOCKOTA FROM ( SELECT [COLLATERAL_ID], [COLLATERAL_STAT], [ASSET_TYPE_CODE], [FULL_ASSET_CODE], [FULL_ASSET_NAME], [MANUFACTURING_YEAR], [MR_COLLATERAL_CONDITION_CODE], [ASSET_TAX_DT], [COLLATERAL_VALUE_AMT], [COLLATERAL_PORTION_PRCNT], [COLLATERAL_PORTION_AMT], [REMAINING_PORTION_PRCNT], [COLLATERAL_NOTES], [SERIAL_NO_1], [SERIAL_NO_2], [SERIAL_NO_3], [SERIAL_NO_4], [SERIAL_NO_5], [OWNERTYPE], [OWNERNAME], [OWNERPROF], [PROFNAME], [OWNERID], [OWNERMOBILE], [OWNERADDR], [OWNERRT], [OWNERRW], [OWNERZIPCODE], [OWNERKECAMATAN], [OWNERKELURAHAN], [OWNERKOTA], [LOCATIONADDR], [LOCATIONRT], [LOCATIONRW], [LOCATIONZIPCODE], [LOCATIONKECAMATAN], [LOCATIONKELURAHAN], [LOCATIONKOTA], mastername.Code, DESCR FROM ( select [COLLATERAL_ID], [COLLATERAL_STAT], [ASSET_TYPE_CODE], [FULL_ASSET_CODE], [FULL_ASSET_NAME], [MANUFACTURING_YEAR], [MR_COLLATERAL_CONDITION_CODE], [ASSET_TAX_DT], [COLLATERAL_VALUE_AMT], [COLLATERAL_PORTION_PRCNT], [COLLATERAL_PORTION_AMT], [REMAINING_PORTION_PRCNT], [COLLATERAL_NOTES], [SERIAL_NO_1], [SERIAL_NO_2], [SERIAL_NO_3], [SERIAL_NO_4], [SERIAL_NO_5], [OWNERTYPE], [OWNERNAME], [OWNERPROF], [PROFNAME], [OWNERID], [OWNERMOBILE], [OWNERADDR], [OWNERRT], [OWNERRW], [OWNERZIPCODE], [OWNERKECAMATAN], [OWNERKELURAHAN], [OWNERKOTA], [LOCATIONADDR], [LOCATIONRT], [LOCATIONRW], [LOCATIONZIPCODE], [LOCATIONKECAMATAN], [LOCATIONKELURAHAN], [LOCATIONKOTA], [Code], value FROM ( SELECT aco.APP_COLLATERAL_ID as [COLLATERAL_ID], aco.COLLATERAL_STAT as [COLLATERAL_STAT], aco.ASSET_TYPE_CODE as [ASSET_TYPE_CODE], aco.FULL_ASSET_CODE as [FULL_ASSET_CODE], aco.FULL_ASSET_NAME as [FULL_ASSET_NAME], aco.MANUFACTURING_YEAR as [MANUFACTURING_YEAR], aco.MR_COLLATERAL_CONDITION_CODE as [MR_COLLATERAL_CONDITION_CODE], aco.MR_COLLATERAL_USAGE_CODE as 'ASSET_USAGE', FORMAT(ASSET_TAX_DT, 'MM/dd/yyy') as [ASSET_TAX_DT], aco.COLLATERAL_VALUE_AMT as [COLLATERAL_VALUE_AMT], aco.COLLATERAL_PORTION_PRCNT as [COLLATERAL_PORTION_PRCNT], aco.COLLATERAL_PORTION_AMT as [COLLATERAL_PORTION_AMT], 100 - aco.COLLATERAL_PORTION_PRCNT as [REMAINING_PORTION_PRCNT], aco.COLLATERAL_NOTES as [COLLATERAL_NOTES], aco.SERIAL_NO_1 as [SERIAL_NO_1], aco.SERIAL_NO_2 as [SERIAL_NO_2], aco.SERIAL_NO_3 as [SERIAL_NO_3], aco.SERIAL_NO_4 as [SERIAL_NO_4], aco.SERIAL_NO_5 as [SERIAL_NO_5], acr.MR_OWNER_TYPE_CODE AS [OWNERTYPE], acr.OWNER_NAME AS [OWNERNAME], acr.MR_OWNER_RELATIONSHIP_CODE AS [OWNERRELATIONSHIP], acr.OWNER_PROFESSION_CODE AS [OWNERPROF], rpl.PROFESSION_NAME AS [PROFNAME], acr.MR_ID_TYPE_CODE AS 'ID_TYPE', acr.OWNER_ID_NO AS [OWNERID], acr.OWNER_MOBILE_PHN_NO AS [OWNERMOBILE], acr.OWNER_ADDR AS [OWNERADDR], acr.OWNER_AREA_CODE_4 AS [OWNERRT], acr.OWNER_AREA_CODE_3 AS [OWNERRW], acr.OWNER_ZIPCODE AS [OWNERZIPCODE], acr.OWNER_AREA_CODE_2 AS [OWNERKECAMATAN], acr.OWNER_AREA_CODE_1 AS [OWNERKELURAHAN], acr.OWNER_CITY AS [OWNERKOTA], acr.LOCATION_ADDR AS [LOCATIONADDR], acr.LOCATION_AREA_CODE_4 AS [LOCATIONRT], acr.LOCATION_AREA_CODE_3 AS [LOCATIONRW], acr.LOCATION_ZIPCODE AS [LOCATIONZIPCODE], acr.LOCATION_AREA_CODE_2 AS [LOCATIONKECAMATAN], acr.LOCATION_AREA_CODE_1 AS [LOCATIONKELURAHAN], acr.LOCATION_CITY AS [LOCATIONKOTA] FROM APP_COLLATERAL aco WITH(NOLOCK) JOIN APP a WITH(NOLOCK) ON aco.APP_ID = a.APP_ID JOIN APP_COLLATERAL_REGISTRATION acr WITH(NOLOCK) ON aco.APP_COLLATERAL_ID = acr.APP_COLLATERAL_ID LEFT JOIN REF_PROFESSION_LOS rpl with(nolock) on rpl.PROFESSION_CODE = acr.OWNER_PROFESSION_CODE WHERE APP_NO = '"+ appno +"' AND aco.IS_MAIN_COLLATERAL = 0 ) as Orig unpivot ( value for [Code] in ( [ASSET_USAGE], [ID_TYPE], [OWNERRELATIONSHIP] ) ) as unpiv ) as mastername JOIN FOUNDATION.dbo.REF_MASTER rf WITH(NOLOCK) ON rf.MASTER_CODE = mastername.value WHERE rf.IS_ACTIVE = '1' AND rf.REF_MASTER_TYPE_CODE IN( 'CUST_PERSONAL_RELATIONSHIP', 'CUST_COMPANY_RELATIONSHIP', 'ID_TYPE', 'ASSET_USAGE' ) ) AS ref PIVOT ( MAX(ref.DESCR) for [Code] in ( [ASSET_USAGE], [CUST_PERSONAL_RELATIONSHIP], [ID_TYPE], [OWNERRELATIONSHIP] ) ) as piv ORDER BY [COLLATERAL_ID] DESC"), {  row ->
	...
	//count existing collateral data
	@Keyword
	public countExistingCollateralCountExisting (Sql instance, String custno){
		String assetinfo
		instance.eachRow(("SELECT count(*) FROM COLLATERAL AS CO WITH (NOLOCK)     INNER JOIN dbo.COLLATERAL_REGISTRATION AS CR WITH (NOLOCK) ON CO.COLLATERAL_ID = CR.COLLATERAL_ID     INNER JOIN dbo.CUST AS CU WITH (NOLOCK) ON CO.CUST_ID = CU.CUST_ID  WHERE    ( CO.COLLATERAL_PRCNT < 100 OR CO.COLLATERAL_PRCNT IS NULL ) AND     CU.CUST_NO = '"+ custno +"'"), {  row ->
	...
	//get existing collateral data masih ambil row 1 dulu karena tidak ada code unik
	@Keyword
	public getDataExistingCollateral (Sql instance, String custno, String taxdate){
		String assetinfo
		ArrayList<String> listassetinfo = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL([FULLASSETCODE], ''), ISNULL([MR_COLLATERAL_CONDITION_CODE], ''), ISNULL([ASSET_USAGE], ''), ISNULL([ASSET_TAX_DT], ''), [COLLATERAL_VALUE_AMT], [COLLATERAL_NOTES], ISNULL([SERIAL_NO_1], ''), ISNULL([SERIAL_NO_2], ''), ISNULL([SERIAL_NO_3], ''), ISNULL([SERIAL_NO_4], ''), ISNULL([SERIAL_NO_5], ''), ISNULL([OWNERNAME], '') AS OWNERNAME, ISNULL([OWNERRELATIONSHIP], '') AS OWNER_RELATION, CASE WHEN LEN(PROFNAME) > 0 THEN PROFNAME WHEN LEN(OWNERPROF) > 0 THEN OWNERPROF END AS OWNER_PROF, ISNULL([ID_TYPE], '') AS IDTYPE, ISNULL([OWNERID], '') AS IDONWER, ISNULL([OWNERMOBILE], '') AS OWNER_MOBILE, ISNULL([OWNERADDR], '') AS OWNERADDR, ISNULL([OWNERRT], '') AS OWNERRT, ISNULL([OWNERRW], '') AS OWNERRW, ISNULL([OWNERZIPCODE], '') AS OWNERZIPCODE, ISNULL([OWNERKECAMATAN], '') AS OWNERKECAMATAN, ISNULL([OWNERKELURAHAN], '') AS OWNERKELURAHAN, ISNULL([OWNERKOTA], '') AS OWNERKOTA, ISNULL([LOCATIONADDR], '') AS LOCADDR, ISNULL([LOCATIONRT], '') AS LOCRT, ISNULL([LOCATIONRW], '') AS LOCRW, ISNULL([LOCATIONZIPCODE], '') AS LOCZIPCODE, ISNULL([LOCATIONKECAMATAN], '') AS LOCKECAMATAN, ISNULL([LOCATIONKELURAHAN], '') AS LOCKELURAHAN, ISNULL([LOCATIONKOTA], '') AS LOCKOTA FROM ( SELECT [FULLASSETCODE], [ASSET_TYPE_CODE], [FULL_ASSET_NAME], [MR_COLLATERAL_CONDITION_CODE], [ASSET_TAX_DT], [COLLATERAL_VALUE_AMT], [COLLATERAL_NOTES], [SERIAL_NO_1], [SERIAL_NO_2], [SERIAL_NO_3], [SERIAL_NO_4], [SERIAL_NO_5], [OWNERTYPE], [OWNERNAME], [OWNERPROF], [PROFNAME], [OWNERID], [OWNERMOBILE], [OWNERADDR], [OWNERRT], [OWNERRW], [OWNERZIPCODE], [OWNERKECAMATAN], [OWNERKELURAHAN], [OWNERKOTA], [LOCATIONADDR], [LOCATIONRT], [LOCATIONRW], [LOCATIONZIPCODE], [LOCATIONKECAMATAN], [LOCATIONKELURAHAN], [LOCATIONKOTA], mastername.Code, DESCR FROM ( select [FULLASSETCODE], [ASSET_TYPE_CODE], [FULL_ASSET_NAME], [MR_COLLATERAL_CONDITION_CODE], [ASSET_TAX_DT], [COLLATERAL_VALUE_AMT], [COLLATERAL_NOTES], [SERIAL_NO_1], [SERIAL_NO_2], [SERIAL_NO_3], [SERIAL_NO_4], [SERIAL_NO_5], [OWNERTYPE], [OWNERNAME], [OWNERPROF], [PROFNAME], [OWNERID], [OWNERMOBILE], [OWNERADDR], [OWNERRT], [OWNERRW], [OWNERZIPCODE], [OWNERKECAMATAN], [OWNERKELURAHAN], [OWNERKOTA], [LOCATIONADDR], [LOCATIONRT], [LOCATIONRW], [LOCATIONZIPCODE], [LOCATIONKECAMATAN], [LOCATIONKELURAHAN], [LOCATIONKOTA], [Code], value FROM ( SELECT CO.FULL_ASSET_CODE AS [FULLASSETCODE], CO.FULL_ASSET_NAME as [FULL_ASSET_NAME], CO.ASSET_TYPE_CODE as [ASSET_TYPE_CODE], CO.MR_COLLATERAL_CONDITION_CODE as [MR_COLLATERAL_CONDITION_CODE], CO.MR_COLLATERAL_USAGE_CODE as 'ASSET_USAGE', FORMAT(ASSET_TAX_DATE, 'yyyy-MM-dd') as [ASSET_TAX_DT], CO.COLLATERAL_PRICE_AMT as [COLLATERAL_VALUE_AMT], CO.NOTES as [COLLATERAL_NOTES], CO.SERIAL_NO_1 as [SERIAL_NO_1], CO.SERIAL_NO_2 as [SERIAL_NO_2], CO.SERIAL_NO_3 as [SERIAL_NO_3], CO.SERIAL_NO_4 as [SERIAL_NO_4], CO.SERIAL_NO_5 as [SERIAL_NO_5], CR.MR_OWNER_TYPE_CODE AS [OWNERTYPE], CR.OWNER_NAME AS [OWNERNAME], CR.MR_OWNER_RELATIONSHIP_CODE AS [OWNERRELATIONSHIP], CR.OWNER_PROFESSION_CODE AS [OWNERPROF], rpl.PROFESSION_NAME AS [PROFNAME], CR.MR_ID_TYPE_CODE AS 'ID_TYPE', CR.OWNER_ID_NO AS [OWNERID], CR.OWNER_MOBILE_PHN_NO AS [OWNERMOBILE], CR.OWNER_ADDR AS [OWNERADDR], CR.OWNER_AREA_CODE_4 AS [OWNERRT], CR.OWNER_AREA_CODE_3 AS [OWNERRW], CR.OWNER_ZIPCODE AS [OWNERZIPCODE], CR.OWNER_AREA_CODE_2 AS [OWNERKECAMATAN], CR.OWNER_AREA_CODE_1 AS [OWNERKELURAHAN], CR.OWNER_CITY AS [OWNERKOTA], CR.LOCATION_ADDR AS [LOCATIONADDR], CR.LOCATION_AREA_CODE_4 AS [LOCATIONRT], CR.LOCATION_AREA_CODE_3 AS [LOCATIONRW], CR.LOCATION_ZIPCODE AS [LOCATIONZIPCODE], CR.LOCATION_AREA_CODE_2 AS [LOCATIONKECAMATAN], CR.LOCATION_AREA_CODE_1 AS [LOCATIONKELURAHAN], CR.LOCATION_CITY AS [LOCATIONKOTA] FROM COLLATERAL AS CO WITH (NOLOCK)     INNER JOIN dbo.COLLATERAL_REGISTRATION AS CR WITH (NOLOCK) ON CO.COLLATERAL_ID = CR.COLLATERAL_ID     INNER JOIN dbo.CUST AS CU WITH (NOLOCK) ON CO.CUST_ID = CU.CUST_ID  LEFT JOIN REF_PROFESSION rpl with(nolock) on rpl.PROFESSION_CODE = CR.OWNER_PROFESSION_CODE WHERE    ( CO.COLLATERAL_PRCNT < 100 OR CO.COLLATERAL_PRCNT IS NULL ) AND CU.CUST_NO = '"+ custno +"' OR FORMAT(ASSET_TAX_DATE, 'yyyy-MM-dd') = '"+ taxdate +"' ) as Orig unpivot ( value for [Code] in ( [ASSET_USAGE], [ID_TYPE], [OWNERRELATIONSHIP] ) ) as unpiv ) as mastername JOIN FOUNDATION.dbo.REF_MASTER rf WITH(NOLOCK) ON rf.MASTER_CODE = mastername.value WHERE rf.IS_ACTIVE = '1' AND rf.REF_MASTER_TYPE_CODE IN( 'CUST_PERSONAL_RELATIONSHIP', 'CUST_COMPANY_RELATIONSHIP', 'ID_TYPE', 'ASSET_USAGE' ) ) AS ref PIVOT ( MAX(ref.DESCR) for [Code] in ( [ASSET_USAGE], [CUST_PERSONAL_RELATIONSHIP], [ID_TYPE], [OWNERRELATIONSHIP] ) ) as piv"), {  row ->
	...
	//get existing collateral data store DB
	@Keyword
	public ExistingCollateralstoreDB (Sql instance, String appno){
		String assetinfo
		ArrayList<String> listassetinfo = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL( [MR_COLLATERAL_CONDITION_CODE], '' ), ISNULL([ASSET_USAGE], ''), ISNULL([ASSET_TAX_DT], ''), [COLLATERAL_VALUE_AMT], [COLLATERAL_NOTES], ISNULL([SERIAL_NO_1], ''), ISNULL([SERIAL_NO_2], ''), ISNULL([SERIAL_NO_3], ''), ISNULL([SERIAL_NO_4], ''), ISNULL([SERIAL_NO_5], ''), ISNULL([OWNERNAME], '') AS OWNERNAME, ISNULL([OWNERRELATIONSHIP], '') AS OWNER_RELATION, CASE WHEN LEN(PROFNAME) > 0 THEN PROFNAME WHEN LEN(OWNERPROF) > 0 THEN OWNERPROF END AS OWNER_PROF, ISNULL([ID_TYPE], '') AS IDTYPE, ISNULL([OWNERID], '') AS IDONWER, ISNULL([OWNERMOBILE], '') AS OWNER_MOBILE, ISNULL([OWNERADDR], '') AS OWNERADDR, ISNULL([OWNERRT], '') AS OWNERRT, ISNULL([OWNERRW], '') AS OWNERRW, ISNULL([OWNERZIPCODE], '') AS OWNERZIPCODE, ISNULL([OWNERKECAMATAN], '') AS OWNERKECAMATAN, ISNULL([OWNERKELURAHAN], '') AS OWNERKELURAHAN, ISNULL([OWNERKOTA], '') AS OWNERKOTA, ISNULL([LOCATIONADDR], '') AS LOCADDR, ISNULL([LOCATIONRT], '') AS LOCRT, ISNULL([LOCATIONRW], '') AS LOCRW, ISNULL([LOCATIONZIPCODE], '') AS LOCZIPCODE, ISNULL([LOCATIONKECAMATAN], '') AS LOCKECAMATAN, ISNULL([LOCATIONKELURAHAN], '') AS LOCKELURAHAN, ISNULL([LOCATIONKOTA], '') AS LOCKOTA FROM ( SELECT [COLLATERAL_ID], [COLLATERAL_STAT], [ASSET_TYPE_CODE], [FULL_ASSET_CODE], [FULL_ASSET_NAME], [MANUFACTURING_YEAR], [MR_COLLATERAL_CONDITION_CODE], [ASSET_TAX_DT], [COLLATERAL_VALUE_AMT], [COLLATERAL_PORTION_PRCNT], [COLLATERAL_PORTION_AMT], [REMAINING_PORTION_PRCNT], [COLLATERAL_NOTES], [SERIAL_NO_1], [SERIAL_NO_2], [SERIAL_NO_3], [SERIAL_NO_4], [SERIAL_NO_5], [OWNERTYPE], [OWNERNAME], [OWNERPROF], [PROFNAME], [OWNERID], [OWNERMOBILE], [OWNERADDR], [OWNERRT], [OWNERRW], [OWNERZIPCODE], [OWNERKECAMATAN], [OWNERKELURAHAN], [OWNERKOTA], [LOCATIONADDR], [LOCATIONRT], [LOCATIONRW], [LOCATIONZIPCODE], [LOCATIONKECAMATAN], [LOCATIONKELURAHAN], [LOCATIONKOTA], mastername.Code, DESCR FROM ( select [COLLATERAL_ID], [COLLATERAL_STAT], [ASSET_TYPE_CODE], [FULL_ASSET_CODE], [FULL_ASSET_NAME], [MANUFACTURING_YEAR], [MR_COLLATERAL_CONDITION_CODE], [ASSET_TAX_DT], [COLLATERAL_VALUE_AMT], [COLLATERAL_PORTION_PRCNT], [COLLATERAL_PORTION_AMT], [REMAINING_PORTION_PRCNT], [COLLATERAL_NOTES], [SERIAL_NO_1], [SERIAL_NO_2], [SERIAL_NO_3], [SERIAL_NO_4], [SERIAL_NO_5], [OWNERTYPE], [OWNERNAME], [OWNERPROF], [PROFNAME], [OWNERID], [OWNERMOBILE], [OWNERADDR], [OWNERRT], [OWNERRW], [OWNERZIPCODE], [OWNERKECAMATAN], [OWNERKELURAHAN], [OWNERKOTA], [LOCATIONADDR], [LOCATIONRT], [LOCATIONRW], [LOCATIONZIPCODE], [LOCATIONKECAMATAN], [LOCATIONKELURAHAN], [LOCATIONKOTA], [Code], value FROM ( SELECT aco.APP_COLLATERAL_ID as [COLLATERAL_ID], aco.COLLATERAL_STAT as [COLLATERAL_STAT], aco.ASSET_TYPE_CODE as [ASSET_TYPE_CODE], aco.FULL_ASSET_CODE as [FULL_ASSET_CODE], aco.FULL_ASSET_NAME as [FULL_ASSET_NAME], aco.MANUFACTURING_YEAR as [MANUFACTURING_YEAR], aco.MR_COLLATERAL_CONDITION_CODE as [MR_COLLATERAL_CONDITION_CODE], aco.MR_COLLATERAL_USAGE_CODE as 'ASSET_USAGE', FORMAT(ASSET_TAX_DT, 'yyyy-MM-dd') as [ASSET_TAX_DT], aco.COLLATERAL_VALUE_AMT as [COLLATERAL_VALUE_AMT], aco.COLLATERAL_PORTION_PRCNT as [COLLATERAL_PORTION_PRCNT], aco.COLLATERAL_PORTION_AMT as [COLLATERAL_PORTION_AMT], 100 - aco.COLLATERAL_PORTION_PRCNT as [REMAINING_PORTION_PRCNT], aco.COLLATERAL_NOTES as [COLLATERAL_NOTES], aco.SERIAL_NO_1 as [SERIAL_NO_1], aco.SERIAL_NO_2 as [SERIAL_NO_2], aco.SERIAL_NO_3 as [SERIAL_NO_3], aco.SERIAL_NO_4 as [SERIAL_NO_4], aco.SERIAL_NO_5 as [SERIAL_NO_5], acr.MR_OWNER_TYPE_CODE AS [OWNERTYPE], acr.OWNER_NAME AS [OWNERNAME], acr.MR_OWNER_RELATIONSHIP_CODE AS [OWNERRELATIONSHIP], acr.OWNER_PROFESSION_CODE AS [OWNERPROF], rpl.PROFESSION_NAME AS [PROFNAME], acr.MR_ID_TYPE_CODE AS 'ID_TYPE', acr.OWNER_ID_NO AS [OWNERID], acr.OWNER_MOBILE_PHN_NO AS [OWNERMOBILE], acr.OWNER_ADDR AS [OWNERADDR], acr.OWNER_AREA_CODE_4 AS [OWNERRT], acr.OWNER_AREA_CODE_3 AS [OWNERRW], acr.OWNER_ZIPCODE AS [OWNERZIPCODE], acr.OWNER_AREA_CODE_2 AS [OWNERKECAMATAN], acr.OWNER_AREA_CODE_1 AS [OWNERKELURAHAN], acr.OWNER_CITY AS [OWNERKOTA], acr.LOCATION_ADDR AS [LOCATIONADDR], acr.LOCATION_AREA_CODE_4 AS [LOCATIONRT], acr.LOCATION_AREA_CODE_3 AS [LOCATIONRW], acr.LOCATION_ZIPCODE AS [LOCATIONZIPCODE], acr.LOCATION_AREA_CODE_2 AS [LOCATIONKECAMATAN], acr.LOCATION_AREA_CODE_1 AS [LOCATIONKELURAHAN], acr.LOCATION_CITY AS [LOCATIONKOTA] FROM APP_COLLATERAL aco WITH(NOLOCK) JOIN APP a WITH(NOLOCK) ON aco.APP_ID = a.APP_ID JOIN APP_COLLATERAL_REGISTRATION acr WITH(NOLOCK) ON aco.APP_COLLATERAL_ID = acr.APP_COLLATERAL_ID LEFT JOIN REF_PROFESSION_LOS rpl with(nolock) on rpl.PROFESSION_CODE = acr.OWNER_PROFESSION_CODE WHERE APP_NO = '"+ appno +"' AND aco.IS_MAIN_COLLATERAL = 0 ) as Orig unpivot ( value for [Code] in ( [ASSET_USAGE], [ID_TYPE], [OWNERRELATIONSHIP] ) ) as unpiv ) as mastername JOIN FOUNDATION.dbo.REF_MASTER rf WITH(NOLOCK) ON rf.MASTER_CODE = mastername.value WHERE rf.IS_ACTIVE = '1' AND rf.REF_MASTER_TYPE_CODE IN( 'CUST_PERSONAL_RELATIONSHIP', 'CUST_COMPANY_RELATIONSHIP', 'ID_TYPE', 'ASSET_USAGE' ) ) AS ref PIVOT ( MAX(ref.DESCR) for [Code] in ( [ASSET_USAGE], [CUST_PERSONAL_RELATIONSHIP], [ID_TYPE], [OWNERRELATIONSHIP] ) ) as piv ORDER BY [COLLATERAL_ID] DESC"), {  row ->
	...
	public NAP2InsuranceMFStoreDB (Sql instance, String appno){
		String insurancedata
		ArrayList<String> insurancelist = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL(aio.INS_ASSET_REGION,'') AS ASSET_REG , ISNULL(CONVERT(INT , aio.CVG_AMT),'') AS CVG_AMT, ISNULL(rml.DESCR,'') AS REF1, ISNULL(SUBQ.DESCR,'') AS REF2, ISNULL(aio.INSCO_BRANCH_NAME,'') AS INSCO_BRANCH_NAME, ISNULL(aio.NOTES,'') AS NOTES, ISNULL(INS_LENGTH,'') AS INS_LENGTH , ISNULL(CAST(CUST_ADMIN_FEE_AMT as int),'') AS ADMIN_FEE_CUST, ISNULL(CAST(CUST_STAMP_DUTY_FEE as INT),'') AS STAMP_DUTY_FEE_CUST FROM APP_INS ai WITH(NOLOCK) JOIN APP a WITH(NOLOCK) ON ai.APP_ID = a.APP_ID JOIN APP_INS_OBJ aio WITH(NOLOCK) ON a.APP_ID = aio.APP_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = aio.INS_ASSET_COVER_PERIOD, (SELECT rml.DESCR, APP_NO FROM APP_INS_OBJ aio WITH(NOLOCK) JOIN APP a WITH(NOLOCK) ON aio.APP_ID = a.APP_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = aio.PAY_PERIOD_TO_INSCO) as SUBQ WHERE a.APP_NO = '"+ appno +"' AND a.APP_NO = SUBQ.APP_NO AND rml.REF_MASTER_TYPE_CODE = 'INS_COVER_PERIOD'"), {  row ->
	...
	public NAP2InsuranceMainCVGtoreDB (Sql instance, String appno){
		String insurancedata
		ArrayList<String> insurancelist = new ArrayList<>()
		instance.eachRow(("SELECT rml.DESCR FROM APP_INS_MAIN_CVG aimc JOIN APP_INS_OBJ aio WITH(NOLOCK) ON aio.APP_INS_OBJ_ID = aimc.APP_INS_OBJ_ID JOIN APP a WITH(NOLOCK) ON a.APP_ID = aio.APP_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = aimc.MR_MAIN_CVG_TYPE_CODE WHERE APP_NO = '"+ appno +"'"), {  row ->
	...
	public NAP2InsuranceMultiMainCVGtoreDB (Sql instance, String appno){
		String insurancedata
		ArrayList<String> insurancelist = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL([YEAR],'') AS YEAR, CASE WHEN IS_CAPITALIZED = 1 THEN 'Yes' WHEN IS_CAPITALIZED = 0 THEN 'No' END AS IS_CPTLZ, ISNULL([PAID],'') AS PAIDBY,  ISNULL([SUMINSURED],'') AS SUMINSURED, ISNULL([MAINCVG],'') AS MAINCVG FROM (SELECT [YEAR], test.Code, DESCR, IS_CAPITALIZED,  [SUMINSURED] FROM (select [YEAR], [Code], value, IS_CAPITALIZED,  [SUMINSURED] from (SELECT YEAR_NO AS [YEAR], IS_CAPITALIZED , MR_INS_PAID_BY_CODE as [PAID], FORMAT(aimc.SUM_INSURED_PRCNT, 'N0') as [SUMINSURED], MR_MAIN_CVG_TYPE_CODE as [MAINCVG] FROM APP_INS_MAIN_CVG aimc JOIN APP_INS_OBJ aio WITH(NOLOCK) ON aio.APP_INS_OBJ_ID = aimc.APP_INS_OBJ_ID JOIN APP a WITH(NOLOCK) ON a.APP_ID = aio.APP_ID WHERE a.APP_NO = '"+ appno +"') as Orig unpivot(value for [Code] in ([PAID],[MAINCVG]) )as unpiv) as test JOIN FOUNDATION.dbo.REF_MASTER rf on rf.MASTER_CODE = test.value) AS A PIVOT (MAX(A.DESCR) for [Code] in ([PAID],[MAINCVG])) as piv"), {  row ->
	...
	public NAP2InsuranceMultiAddCVGtoreDB (Sql instance, String appno){
		String insurancedata
		ArrayList<String> insurancelist = new ArrayList<>()
		instance.eachRow(("SELECT ISNULL(YEAR_NO,'') AS YEARNO,ISNULL(rml.DESCR,'') AS ADD_CVG, ISNULL(CASE WHEN(rml.DESCR = 'TPL' OR rml.DESCR = 'Tanggung Jawab Hukum Terhadap Penumpang' OR rml.DESCR = 'Kecelakaan Diri Untuk Penumpang') THEN CONVERT(VARCHAR,CUST_ADD_PREMI_AMT) ELSE CONVERT(VARCHAR,CUST_ADD_PREMI_RATE) END,'') AS RATE FROM APP_INS_MAIN_CVG aimc JOIN APP_INS_OBJ aio WITH(NOLOCK) ON aio.APP_INS_OBJ_ID = aimc.APP_INS_OBJ_ID JOIN APP a WITH(NOLOCK) ON a.APP_ID = aio.APP_ID JOIN APP_INS_ADD_CVG aiac WITH(NOLOCK) ON aiac.APP_INS_MAIN_CVG_ID = aimc.APP_INS_MAIN_CVG_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = aiac.MR_ADD_CVG_TYPE_CODE WHERE APP_NO = '"+ appno +"' ORDER BY YEAR_NO,APP_INS_ADD_CVG_ID"), {  row ->
			//SELECT YEAR_NO ,rml.DESCR, CASE WHEN(rml.DESCR = 'TPL' OR rml.DESCR = 'Tanggung Jawab Hukum Terhadap Penumpang' OR rml.DESCR = 'Kecelakaan Diri Untuk Penumpang') THEN CUST_ADD_PREMI_AMT ELSE FORMAT(CUST_ADD_PREMI_RATE, 'N4') END FROM APP_INS_MAIN_CVG aimc JOIN APP_INS_OBJ aio WITH(NOLOCK) ON aio.APP_INS_OBJ_ID = aimc.APP_INS_OBJ_ID JOIN APP a WITH(NOLOCK) ON a.APP_ID = aio.APP_ID JOIN APP_INS_ADD_CVG aiac WITH(NOLOCK) ON aiac.APP_INS_MAIN_CVG_ID = aimc.APP_INS_MAIN_CVG_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = aiac.MR_ADD_CVG_TYPE_CODE WHERE APP_NO = '"+ appno +"' ORDER BY YEAR_NO
			ResultSetMetaData rsmd = row.getMetaData()
			colmcount = rsmd.getColumnCount()
	...
	public NAP2FinancialStoreDB (Sql instance, String appno){
		...
		instance.eachRow(("SELECT ISNULL(CONVERT(INT, TOTAL_FEE_AMT),'') AS TOTAL_FEE, ISNULL(CONVERT(INT, TOTAL_FEE_CPTLZ_AMT),'') AS TOTAL_FEE_CPTLZ, ISNULL(rml.DESCR,'') AS PROV_FEE_CALC_METHOD, ISNULL(EFFECTIVE_RATE_PRCNT,'') AS EF_RATE, ISNULL(FLAT_RATE_PRCNT,'') AS FLAT_RATE, ISNULL(GRACE_PERIOD,'') AS GRACE_PERIOD, ISNULL(afd.MR_GRACE_PERIOD_TYPE_CODE,'') AS GRACE_PERIOD_TYPE, ISNULL(CONVERT(INT,TDP_PAID_COY_AMT),'') AS TDP_PAID_COY_AMT FROM APP_FIN_DATA afd WITH(NOLOCK) JOIN APP a WITH(NOLOCK) ON a.APP_ID = afd.APP_ID JOIN FOUNDATION.dbo.REF_MASTER rml ON rml.MASTER_CODE = afd.MR_PROVISION_FEE_CALC_METHOD_CODE WHERE APP_NO = '"+ appno +"'"), {  row ->
	...
	public NAP4FinancialDataPersonalStoreData (Sql instance, String appno, String name, String date){
		...
		instance.eachRow(("SELECT ISNULL(FORMAT(MONTHLY_INCOME_AMT, 'N0'),'') AS MONINCOME, ISNULL(FORMAT(OTHER_INCOME_AMT, 'N0'),'') AS OTHINCOME, ISNULL(rml.DESCR,'') AS SOURCEINCOME, ISNULL(FORMAT(MONTHLY_INSTALLMENT_AMT, 'N0'),'') AS MONTHINST, ISNULL(FORMAT(MONTHLY_EXPENSE_AMT, 'N0'),'') AS MONEXPN, ISNULL(FORMAT(acpfd.DATE_AS_OF , 'MM/dd/yyy'),'') AS DATEASOF FROM APP_CUST_PERSONAL_FIN_DATA acpfd WITH(NOLOCK) JOIN APP_CUST_PERSONAL acp WITH(NOLOCK) ON acp.APP_CUST_PERSONAL_ID = acpfd.APP_CUST_PERSONAL_ID JOIN APP_CUST ac WITH(NOLOCK) ON ac.APP_CUST_ID = acp.APP_CUST_ID JOIN APP a WITH(NOLOCK) ON a.APP_ID = ac.APP_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = acpfd.MR_SOURCE_OF_INCOME_TYPE_CODE WHERE a.APP_NO = '"+ appno +"' AND ac.CUST_NAME = '"+ name +"' AND FORMAT(acpfd.DATE_AS_OF , 'MM/dd/yyy') = '"+ date +"'"), {  row ->
	...
	public NAP4CustomerAssetDataStoreData (Sql instance, String appno, String name){
		...
		instance.eachRow(("SELECT ISNULL(rml.DESCR,'') AS ASSETTYPE, ISNULL(aca.ASSET_DESCR,'') AS DESCR, ISNULL(FORMAT(aca.ASSET_VALUE, 'N0'),'') AS VAL, ISNULL(aca.ASSET_QTY,'') AS QTY FROM APP_CUST_ASSET aca WITH(NOLOCK) JOIN APP_CUST ac WITH(NOLOCK) ON ac.APP_CUST_ID = aca.APP_CUST_ID JOIN  APP a WITH(NOLOCK) ON a.APP_ID = ac.APP_ID JOIN FOUNDATION.dbo.REF_MASTER rml WITH(NOLOCK) ON rml.MASTER_CODE = aca.MR_CUST_ASSET_TYPE_CODE WHERE a.APP_NO = '"+ appno +"' AND ac.CUST_NAME = '"+ name +"' ORDER BY aca.APP_CUST_ASSET_ID"), {  row ->
