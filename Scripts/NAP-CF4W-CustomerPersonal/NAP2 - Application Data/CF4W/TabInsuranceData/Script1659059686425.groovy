...
if (GlobalVariable.Role == 'Testing') {
	...
	if(GlobalVariable.FirstTimeEntry=="Yes"){
		...
		checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getText(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/CF4W/label_AssetPrice')).replace('.00',''), findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofColm, 88), false))
	
		'Verifikasi perhitungan asset price incl accessories'
		checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getText(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/CF4W/label_AssetPriceInclAcc')).replace('.00',''), findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofColm, 89), false))
	}
	...
	int totalddlinsuredby = WebUI.getNumberOfTotalOption(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/CF4W/select_InsuredBy'))
	...
	if (WebUI.verifyOptionsPresent(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/CF4W/select_InsuredBy'),
		insuredby) == false) {

		'Write To Excel GlobalVariable.StatusFailed and GlobalVariable.ReasonFailedDDL'
		CustomKeywords.'customizeKeyword.writeExcel.writeToExcelStatusReason'('8.TabInsuranceData', GlobalVariable.NumofColm, GlobalVariable.StatusFailed, findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofColm, 2) + ';'+GlobalVariable.ReasonFailedDDL + 'insuredby')
...
WebUI.selectOptionByLabel(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/CF4W/select_InsuredBy'), 
    insuredBy, false)

'Verifikasi nilai insured by'
if (insuredBy == 'Customer') {
    'Memanggil test case Tab Insurance Data Customer'
    WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/CF4W/TabInsuranceDataCustomer'), 
    ...
    WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/CF4W/TabInsuranceDataMultifinance'), 
        [:], FailureHandling.CONTINUE_ON_FAILURE)
...
if (WebUI.verifyElementPresent(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/CF4W/select_InsuredBy'), 
	...
	if (GlobalVariable.Role == 'Testing' && GlobalVariable.CheckVerifStoreDBPersonal=="Yes") {
		...
		WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/CF4W/TabInsuranceDataStoreDBVerif'),
			[:], FailureHandling.CONTINUE_ON_FAILURE)
	}
}
