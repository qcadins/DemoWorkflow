...
if (GlobalVariable.Role == 'Testing') {
	...
	if(GlobalVariable.FirstTimeEntry=="Yes"){
		...
		checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getText(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/FL4W/label_AssetPrice')).replace('.00',''), findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 88), false))
	
		'Verifikasi perhitungan asset price incl accessories'
		checkVerifyEqualOrMatch(WebUI.verifyMatch(WebUI.getText(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/FL4W/label_AssetPriceInclAcc')).replace('.00',''), findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 89), false))
	}
	...
	int totalddlinsuredby = WebUI.getNumberOfTotalOption(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/FL4W/select_InsuredBy'))
	...
	if (WebUI.verifyOptionsPresent(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/FL4W/select_InsuredBy'),
		insuredby) == false) {

		'Write To Excel GlobalVariable.StatusFailed and GlobalVariable.ReasonFailedDDL'
		CustomKeywords.'customizeKeyword.writeExcel.writeToExcelStatusReason'('8.TabInsuranceData', GlobalVariable.NumofInsurance, GlobalVariable.StatusFailed, findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 2) + ';'+GlobalVariable.ReasonFailedDDL + 'insuredby')
...
String insuredBy = findTestData(excelPathTabInsurance).getValue(
    GlobalVariable.NumofInsurance, 12)

'Select option dropdownlist insured by'
WebUI.selectOptionByLabel(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/FL4W/select_InsuredBy'), 
    insuredBy, false)

'Verifikasi nilai insured by'
if (insuredBy == 'Customer') {
    'Memanggil test case Tab Insurance Data Customer'
    WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/FL4W/TabInsuranceDataCustomer'), 
    ...
    WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/FL4W/TabInsuranceDataMultifinance'), 
        [:], FailureHandling.CONTINUE_ON_FAILURE)
...
'Klik save'
WebUI.click(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/FL4W/button_Save'))

WebUI.delay(2)

Integer iscompleteMandatory = Integer.parseInt(findTestData(excelPathTabInsurance).getValue(
        GlobalVariable.NumofInsurance, 4))

if (iscompleteMandatory == 0 && GlobalVariable.FlagFailed == 0) {
    'cek alert'
    GlobalVariable.FlagFailed = CustomKeywords.'checkSaveProcess.checkSaveProcess.checkAlert'(GlobalVariable.NumofInsurance, 
        '8.TabInsuranceData')
}
...
if (GlobalVariable.FlagFailed == 0) {
    ...
    CustomKeywords.'checkSaveProcess.checkSaveProcess.checkStatus'(iscompleteMandatory, findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabLifeInsuranceData/checkbox_coverlifeinsurance'), 
        GlobalVariable.NumofInsurance, '8.TabInsuranceData')

    if (iscompleteMandatory == 0) {
        errorValObject = findTestObject('NAP-CF4W-CustomerPersonal/NAP1-CustomerData/TabCustomerData/div_errorvalidation')

        'cek validasi'
        CustomKeywords.'checkSaveProcess.checkSaveProcess.checkValidasi'(errorValObject, GlobalVariable.NumofInsurance, '8.TabInsuranceData')
    }
}
...
'Verify input data'
if (WebUI.verifyElementPresent(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabInsuranceData/FL4W/select_InsuredBy'), 
	...
	if (GlobalVariable.Role == 'Testing' && GlobalVariable.CheckVerifStoreDBPersonal=="Yes") {
		...
		WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/FL4W/TabInsuranceDataStoreDBVerif'),
			[:], FailureHandling.CONTINUE_ON_FAILURE)
	}
}
