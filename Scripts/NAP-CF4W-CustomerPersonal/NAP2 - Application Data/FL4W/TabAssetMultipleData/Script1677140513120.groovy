...
for (GlobalVariable.NumofAsset = 2; GlobalVariable.NumofAsset <= (findTestData(excelPathTabAsset).getColumnNumbers() - 1); (GlobalVariable.NumofAsset)++) {
    if (findTestData(excelPathTabAsset).getValue(GlobalVariable.NumofAsset, 8) == findTestData(excelPathCustomerPersonal).getValue(
...
for (GlobalVariable.NumofAsset = GlobalVariable.StartIndex; GlobalVariable.NumofAsset <= (countAccessories - 1); (GlobalVariable.NumofAsset)++) {
    if (findTestData(excelPathTabAsset).getValue(GlobalVariable.NumofAsset, 8).equalsIgnoreCase(findTestData(excelPathCustomerPersonal).getValue(
            GlobalVariable.NumofColm, 13))) {
        'click button add'
        WebUI.click(findTestObject('NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabAssetData/AssetMultiple/buttonAddAsset'))

		'call Test case input Asset data'
        WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/FL4W/TabAssetData'), [:], FailureHandling.CONTINUE_ON_FAILURE)
    }
}
