...
String insuredBy = findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 12)
...
if (arrayMatch.contains(false)) {
    ...
    CustomKeywords.'customizeKeyword.writeExcel.writeToExcelStatusReason'('8.TabInsuranceData', GlobalVariable.NumofInsurance, 
        GlobalVariable.StatusFailed, findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 2) + ';'+GlobalVariable.ReasonFailedStoredDB)
}
...
def insuredCust(ArrayList<String> arrayMatch, Sql sqlconnectionLOS) {
    ...
    for (int index = 14; index < (result.size() + 14); index++) {
        ...
        arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, index).toUpperCase(), (result[
...
def insuredCustMF(ArrayList<String> arrayMatch, Sql sqlconnectionLOS, Sql sqlconnectionFOU, int arrayindex) {
	...
	for (int index = 14; index <= (resultCustomerInsurance.size() + 13); index++) {
		...
		if ((index - 13) != resultCustomerInsurance.size()) {
			...
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, index).toUpperCase().replace(',', ''), (resultCustomerInsurance[arrayindex++]).toUpperCase(),
					false, FailureHandling.OPTIONAL))
		} else if ((index - 13) == resultCustomerInsurance.size()) {
			
			String countDate = CustomKeywords.'customizeKeyword.convertDate.countDateInsurance'(findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 19))
	...
	arrayindex = 0
			...
			'verify asset region'
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 22).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
					false, FailureHandling.OPTIONAL))
			
			'verify coverage amount'
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 23).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
					false, FailureHandling.OPTIONAL))
			
			'verify cover period'
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 24).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
					false, FailureHandling.OPTIONAL))
			
			'verify payment type'
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 25).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
					false, FailureHandling.OPTIONAL))
			
			'verify insco branch name'
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 26).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
					false, FailureHandling.OPTIONAL))
			
			'verify insurance note'
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 27).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
					false, FailureHandling.OPTIONAL))
			
			if ((findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 24) == 'Partial Tenor') || (findTestData(excelPathTabInsurance).getValue(
							GlobalVariable.NumofInsurance, 24) == 'Over Tenor')) {
				'verify insurance length'
				arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
						GlobalVariable.NumofInsurance, 28).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
						false, FailureHandling.OPTIONAL))
			} else if ((findTestData(excelPathTabInsurance).getValue(
					GlobalVariable.NumofInsurance, 24) == 'Annualy') || (findTestData(excelPathTabInsurance).getValue(
							GlobalVariable.NumofInsurance, 24) == 'Full Tenor')) {
	...
	'verify admin fee'
	arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
			GlobalVariable.NumofInsurance, 31).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
			false, FailureHandling.OPTIONAL))
	
	'verify customer stampduty fee'
	arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
			GlobalVariable.NumofInsurance, 32).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
			false, FailureHandling.OPTIONAL))
	...
	arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
			GlobalVariable.NumofInsurance, 34).toUpperCase().replace(',', ''), (resultMainCVG[0]).toUpperCase(), false,
	...
	for(i = rowAddCvg+2; i <= rowEditInsTable; i++){
		if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, i).equalsIgnoreCase('Yes')) {
			for (int index = 0; index < resultAddCVG.size(); index++) {
				if ((resultAddCVG[index]).equalsIgnoreCase(findTestData(excelPathTabInsurance).getValue(1, i).replace('$',''))) {
					arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
							GlobalVariable.NumofInsurance, i), 'YES', false, FailureHandling.OPTIONAL))
	...
	String capinssetting = CustomKeywords.'insuranceData.checkCapitalizeSetting.checkInsuranceCapSetting'(sqlconnectionFOU)
			...
			if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+2).length() > 0 || findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+3).length() > 0
					|| findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+4).length() > 0 || findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+5).length() > 0) {
				...
				ArrayList<String> resultMultiMainCVG = CustomKeywords.'dbConnection.CustomerDataVerif.NAP2InsuranceMultiMainCVGtoreDB'(
						...
						def capitalizedarray = findTestData(excelPathTabInsurance).getValue(
								GlobalVariable.NumofInsurance, rowEditInsTable+2).split(';', -1)
						
						def paidbyarray = findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance,
								rowEditInsTable+3).split(';', -1)
						
						def suminsuredarray = findTestData(excelPathTabInsurance).getValue(
								GlobalVariable.NumofInsurance, rowEditInsTable+4).split(';', -1)
						
						def maincvgarray = findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance,
						...
						for (int index = 0; index < (resultMultiMainCVG.size() / 5); index++) {
							...
							if (capinssetting.equalsIgnoreCase('YEARLY')) {
								'verif capitalized'
								if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+2).length() > 0) {
							...
							'verif paid by'
							if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+3).length() > 0) {
							...
							'verif sum insured'
							if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+4).length() > 0) {
							...
							'verif main cvg'
							if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, rowEditInsTable+5).length() > 0) {
	...
	if(capinssetting.equalsIgnoreCase('PARTIAL')){
		if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance,
			...
			ArrayList<String> resultPartialCaptilized = CustomKeywords.'dbConnection.CustomerDataVerif.NAP2InsurancePartialCaptilizeStoreDB'(
					...
					arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
							GlobalVariable.NumofInsurance, 85), (resultPartialCaptilized[0]).replace('.00', ''), false,
							FailureHandling.OPTIONAL))
		}
	}
	...
	int rowEditAddCvg = CustomKeywords.'customizeKeyword.getRow.getExcelRow'(GlobalVariable.DataFilePath, '8.TabInsuranceData', 'Edit Additional Coverage')
			...
			for(i = rowEditAddCvg+2; i <= rowSuminsuredAmt; i++){
				if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, i).length() > 0) {
					
					ArrayList<String> AddRate = GlobalVariable.AdditionalPremiRate
							
							def arrayAddCvg = findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, i).split(';', -1)
	...
	if (findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 69).length() > 0) {
		ArrayList<String> MainRate = GlobalVariable.MainPremiRate
				...
				def mainpremirateArray = findTestData(excelPathTabInsurance).getValue(
						GlobalVariable.NumofInsurance, 69).split(';', -1)
...
def insuredMF(ArrayList<String> arrayMatch, Sql sqlconnectionLOS, Sql sqlconnectionFOU) {
	arrayindex = 0
			...
			if(findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 27).length() > 0){
				'verify insurance note'
				arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(
						GlobalVariable.NumofInsurance, 27).toUpperCase().replace(',', ''), (resultMFinsurance[arrayindex++]).toUpperCase(),
						false, FailureHandling.OPTIONAL))
	...
	if(findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 48).length() == 0){
		for(int cvg = 0; cvg < resultMainCVG.size(); cvg++){
			'verify main coverage'
			arrayMatch.add(WebUI.verifyMatch(findTestData(excelPathTabInsurance).getValue(GlobalVariable.NumofInsurance, 34).toUpperCase(), resultMainCVG[cvg].toUpperCase(), false, FailureHandling.OPTIONAL))
		}
	}
