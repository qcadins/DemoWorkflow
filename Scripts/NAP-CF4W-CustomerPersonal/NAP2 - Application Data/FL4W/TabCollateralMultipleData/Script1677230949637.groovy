...
1); (GlobalVariable.NumofCollateral)++) {
    if (findTestData(excelPathCollateral).getValue(GlobalVariable.NumofCollateral, 8) == findTestData(excelPathCustomerPersonal).getValue(
...
for (GlobalVariable.NumofCollateral = GlobalVariable.StartIndex; GlobalVariable.NumofCollateral <= (countCollateral - 1); (GlobalVariable.NumofCollateral)++) {
    if (findTestData(excelPathCollateral).getValue(GlobalVariable.NumofCollateral, 8).equalsIgnoreCase(findTestData(excelPathCustomerPersonal).getValue(
            GlobalVariable.NumofColm, 13))) {
        'click button add'
        WebUI.click(findTestObject('Object Repository/NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabAssetData/Collateral/button_AddCollateral'))
		
        'call Test case input Asset data'
        WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerPersonal/NAP2 - Application Data/FL4W/Collateral'), [:], FailureHandling.CONTINUE_ON_FAILURE)
    }
}

'click button save'
WebUI.click(findTestObject('Object Repository/NAP-CF4W-CustomerPersonal/NAP2-ApplicationData/TabAssetData/Collateral/button_SaveMain'))