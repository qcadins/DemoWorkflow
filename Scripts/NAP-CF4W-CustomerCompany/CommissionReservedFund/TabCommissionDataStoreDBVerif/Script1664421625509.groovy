...
String appno = datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 13)
...
HashMap<String, ArrayList> result = CustomKeywords.'dbConnection.CustomerDataVerif.NAP3CommissionDataStoreDB'(sqlconnection, 
    appno, datafileCommission.getValue(GlobalVariable.NumofColm, 12))
...
if (datafileCommission.getValue(GlobalVariable.NumofColm, 12).equalsIgnoreCase('Amount')) {
    if (suppSource != null) {
        for (int i = 1; i <= suppSource; i++) {
            ...
            if ((datafileCommission.getValue(GlobalVariable.NumofColm, ((2 * i) + 1) + supRow) != '') && (comSupp.get(i - 
                1) != '-1')) {
                'Verif amount commmision supplier db dengan excel'
                arrayMatch.add(WebUI.verifyEqual(Double.parseDouble(comSupp.get(i - 1).toString()), Double.parseDouble(datafileCommission.getValue(
    ...
    if (suppSource != null) {
        for (int i = 1; i <= suppSource; i++) {
            ...
            if ((datafileCommission.getValue(GlobalVariable.NumofColm, ((2 * i) + 2) + supRow) != '') && (comSupp.get(i - 
                1) != '-1')) {
                ...
                arrayMatch.add(WebUI.verifyEqual(Math.round(Double.parseDouble(comSupp.get(i - 1).toString()) * 100) / 100, 
                        Double.parseDouble(datafileCommission.getValue(GlobalVariable.NumofColm, ((2 * i) + 2) + supRow))))
            }
        }
    }
