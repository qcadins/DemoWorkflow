		...
		if(GlobalVariable.CheckRuleCompany=="Yes" ){
			...
			if(WebUI.verifyMatch(textIncomeInfo, ".*"+refundFrom[i]+".*",true)){
				...
				if(WebUI.verifyEqual(Math.round(Double.parseDouble(textIncomeInfoAmt.replace(",",""))),Math.round(getAmountFromAppDB*Double.parseDouble(refundAmt[i])))==false){
					...
					CustomKeywords.'customizeKeyword.writeExcel.writeToExcelStatusReason'('12.TabCommissionData', GlobalVariable.NumofColm, GlobalVariable.StatusFailed, findTestData(excelPathCommission).getValue(GlobalVariable.NumofColm, 2) + ';' + GlobalVariable.ReasonFailedVerifyRule)
...
if(commissiondelete.size() > 0){
	...
	CustomKeywords.'customizeKeyword.writeExcel.writeToExcelStatusReason'('12.TabCommissionData', GlobalVariable.NumofColm, GlobalVariable.StatusWarning, findTestData(excelPathCommission).getValue(GlobalVariable.NumofColm, 2) + ';' + GlobalVariable.ReasonFailedDelete + commissiondelete)
...
public checkVerifyEqualOrMatch(Boolean isMatch, String sheetname, int numofcolm){
		if(isMatch==false && GlobalVariable.FlagFailed==0){
			...
			CustomKeywords.'customizeKeyword.writeExcel.writeToExcelStatusReason'(sheetname, numofcolm, GlobalVariable.StatusFailed, findTestData(excelPathCommission).getValue(GlobalVariable.NumofColm, 2) + ';' + GlobalVariable.ReasonFailedVerifyEqualOrMatch)
