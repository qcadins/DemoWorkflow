...
if (GlobalVariable.FlagFailed == 0) {
	...
	CustomKeywords.'checkSaveProcess.checkSaveProcess.checkStatus'(Integer.parseInt(GlobalVariable.FindDataFile.getValue(
				GlobalVariable.NumofMS, 4)), findTestObject('NAP-CF4W-CustomerCompany/NAP4-CustomerDataCompletion-Company/CustomerCompany/OtherAttribute/button_Debtor Group_btn btn-raised btn-primary'),
...
if (GlobalVariable.FlagWarning > 0) {
	'write to excel status warning'
	CustomKeywords.'customizeKeyword.writeExcel.writeToExcel'(GlobalVariable.DataFilePath, '6.LegalDocument', 0, GlobalVariable.NumofMS -
		1, GlobalVariable.StatusWarning)

	'write to excel failed reason'
	CustomKeywords.'customizeKeyword.writeExcel.writeToExcel'(GlobalVariable.DataFilePath, '6.LegalDocument', 1, GlobalVariable.NumofMS -
...
if ((GlobalVariable.RoleCompany == 'Testing') && (GlobalVariable.CheckVerifStoreDBCompany == 'Yes')) {
	'declare numofverifstore = NumofMS'
...
def inputLegalDoc(ArrayList<String> faileddata){
	def LegalDocTypeArray = GlobalVariable.FindDataFile.getValue(GlobalVariable.NumofMS, 12).split(';', -1)
	
	def DocumentNoArray = GlobalVariable.FindDataFile.getValue(GlobalVariable.NumofMS, 13).split(';', -1)
	
	def DateIssuedArray = GlobalVariable.FindDataFile.getValue(GlobalVariable.NumofMS, 14).split(';', -1)
	
	def ExpiredDateArray = GlobalVariable.FindDataFile.getValue(GlobalVariable.NumofMS, 15).split(';', -1)
	
	def NotaryNameArray = GlobalVariable.FindDataFile.getValue(GlobalVariable.NumofMS, 16).split(';', -1)
	
	def NotaryLocationArray = GlobalVariable.FindDataFile.getValue(GlobalVariable.NumofMS, 17).split(';', -1)
	
	def NotesArray = GlobalVariable.FindDataFile.getValue(GlobalVariable.NumofMS, 18).split(';', -1)
