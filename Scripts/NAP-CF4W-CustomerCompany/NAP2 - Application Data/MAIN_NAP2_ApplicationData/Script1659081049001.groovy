...
if (GlobalVariable.RoleCompany == 'Data Entry') {
	...
	if(datafileReferantor.getValue(copyAppColm, 10).equalsIgnoreCase("No") || datafileReferantor.getValue(copyAppColm, 10).equalsIgnoreCase("Edit")){
		...
		WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerCompany/NAP2 - Application Data/TabReferantorData'),
	...
	if(datafileTabApplication.getValue(GlobalVariable.NumofColm, 10).equalsIgnoreCase("No") || datafileTabApplication.getValue(GlobalVariable.NumofColm, 10).equalsIgnoreCase("Edit")){
	...
	if(datafileTabAsset.getValue(GlobalVariable.NumofColm, 10).equalsIgnoreCase("No") || datafileTabAsset.getValue(GlobalVariable.NumofColm, 10).equalsIgnoreCase("Edit")){
		'call test case tab asset data'
		WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerCompany/NAP2 - Application Data/TabAssetData'),
						[:], FailureHandling.CONTINUE_ON_FAILURE)
	...
	if(datafileTabInsurance.getValue(GlobalVariable.NumofColm, 10).equalsIgnoreCase("No") || datafileTabInsurance.getValue(
		...
		WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerCompany/NAP2 - Application Data/TabInsuranceData'),
						[:], FailureHandling.CONTINUE_ON_FAILURE)
	...
	//dijalankan dengan copy app tab insurance
		...
		if(WebUI.verifyElementPresent(findTestObject('NAP-CF4W-CustomerCompany/NAP2-ApplicationData/TabInsuranceData/button_Calculate Insurance'),3,FailureHandling.OPTIONAL)){
	...
	if(datafileTabFinancial.getValue(GlobalVariable.NumofColm, 10).equalsIgnoreCase("No") || datafileTabFinancial.getValue(
		...
		WebUI.callTestCase(findTestCase('NAP-CF4W-CustomerCompany/NAP2 - Application Data/TabFinancialData'),
						[:], FailureHandling.CONTINUE_ON_FAILURE)
	}
	...
	if((Integer.parseInt(GlobalVariable.CountofReferantorCompany) > 0)){
		...
		1); (GlobalVariable.NumofReferantor)++) {
			if (datafileReferantor.getValue(
				GlobalVariable.NumofReferantor, 12) == datafileCustomerCompany.getValue(
				GlobalVariable.NumofColm, 13)) {
					copyAppColm = GlobalVariable.NumofReferantor
					break
...
public verifyMatch(){
	...
	Boolean isMatch = CustomKeywords.'dbConnection.getInfoForEditNAP.verifyMatchCopyAppYesNAP2Company'(sqlConnectionLOS, datafileCustomerCompany.getValue(
	GlobalVariable.NumofColm, 9),datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 13))
...
public pagingTesting(){
	if(GlobalVariable.RoleCompany=="Testing" && GlobalVariable.CheckPagingCompany=="Yes"){
		...
		String[] textCountData = WebUI.getText(findTestObject('Object Repository/NAP-CF4W-CustomerCompany/NAP2-ApplicationData/countData')).replace(' ', '').replace(':', ';').split(';')
		...
		if(countDt>rowData.size()){
			...
			'verif page 2 active'
			checkVerifyFooter.add(WebUI.verifyElementHasAttribute(findTestObject('Object Repository/NAP-CF4W-CustomerCompany/NAP2-ApplicationData/nextPage'),'aria-current',2))
		...
		if(resultReset.contains(false) && GlobalVariable.StatusFailed!=datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 1)){
				...
				CustomKeywords.'customizeKeyword.writeExcel.writeToExcel'(GlobalVariable.DataFilePath, '1.TabCustomerMainData',
					1, GlobalVariable.NumofColm-1, datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 2).replace("-","")+GlobalVariable.ReasonFailedReset+"NAP2"+";\n")
		...
		if(checkVerifySort.contains(false) && GlobalVariable.StatusFailed!=datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 1)){
				...
				'write to excel reason failed footer'
				CustomKeywords.'customizeKeyword.writeExcel.writeToExcel'(GlobalVariable.DataFilePath, '1.TabCustomerMainData',
					1, GlobalVariable.NumofColm-1, datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 2).replace("-","")+(GlobalVariable.ReasonFailedSort+"NAP2"+";\n"))
		...
		if(checkVerifyFooter.contains(false) && GlobalVariable.StatusFailed!=datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 1)){
				...
				'write to excel reason failed footer'
				CustomKeywords.'customizeKeyword.writeExcel.writeToExcel'(GlobalVariable.DataFilePath, '1.TabCustomerMainData',
					1, GlobalVariable.NumofColm-1, datafileCustomerCompany.getValue(GlobalVariable.NumofColm, 2).replace("-","")+(GlobalVariable.ReasonFailedFooter+"NAP2"+";\n"))
